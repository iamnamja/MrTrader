"""Temp script: factor WF v7 — top_n=5, 20d momentum. Ultra-concentrated."""
import logging
import os
import sys
...
logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s [factor_wf_v7] %(message)s")
...
def main() -> int:
    ...
    logger.info("Factor WF v7: top_n=5, 20d momentum, VIX≤30, SPY-200DMA")
    ...
    logger.info("v7 %s — avg=%.3f min=%.3f folds=%s", verdict, avg_sh, min_sh,
    ...
    try:
        ...
        _smtp_send(
            subject=f"MrTrader Factor WF v7: {verdict} (avg={avg_sh:.3f})",
            html_body=f"<h2>Factor WF v7 (top_n=5, 20d mom)</h2><p><b>{verdict}</b> avg={avg_sh:.3f} min={min_sh:.3f}</p><pre>{fold_rows}</pre>",
        )
