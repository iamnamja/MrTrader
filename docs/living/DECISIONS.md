...
**The week's turn is spent when we ACTUALLY rebalanced — not when the calendar says we could have.** This is the whole correctness argument, and the first cut got it wrong. That cut inferred "the turn has passed" from the static calendar alone, which silently re-created the bug: if the anchor was a trading day but the rebalance was **declined** that morning — a transient Alpaca clock error, or an unscheduled closure absent from the static holiday list (NYSE 2025-01-09 Carter funeral, 2018-12-05, Sandy 2012) — the next day still read "this week's turn was Monday" and skipped, producing exactly the 14-day gap the change exists to remove. The authority is now `back_validation.last_rebalance_date()`, which records only genuine live rebalances; the calendar is the fallback when that lookup fails, degrading to the old conservative behaviour (a missed anchor spends the week), which is the safe direction.
