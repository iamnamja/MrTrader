...
**Decision**: log it as a **PROMISING LEAD, NOT a deploy.** Four reasons not to act yet: (1) thin sample (7-8 folds / 2y, BEAR:1/NEUTRAL:6 — no bull coverage; DSR saturated → no selection screen); (2) single threshold (1.0) → multiplicity/overfitting risk; (3) **residual-α/beta unchecked** — the EventEdge/PEAD CPCV harness doesn't populate daily_returns_dated so no CAPM residual-α was computed, and PEAD's base edge is beta-driven, so the lift could be beta not alpha; (4) neither arm clears the significance gate. Confirmation path before any live change: run pead_phase1_attribution (CAPM beta-isolation) on filtered-vs-baseline, test threshold robustness (0.75/1.25), and ideally more data. The PEAD scorer hook ships **default OFF** (byte-identical committed config).
