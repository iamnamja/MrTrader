...
**Consequence.** 2026-09-08's report becomes a clean PASS under the new rule (pinned by a test using that exact config).

**A second, worse gap fell out of writing that test.** The first version looped over a hardcoded `("whole_book_gate", "reconciliation", "per_name_gate")` tuple instead of `HOLD_REASONS`, which made both of its assertions tautologies — and hid the thing it was supposed to find: **`pm.kill_switch_sm_mode` can HOLD the trend rebalance** (`block_reason='kill_switch_sm'`) **and was monitored by nothing at all.** Set it to `off` and the weekly email still said PASS. It is now in `EXPECT_MIN_MODE` with a recorded minimum of `shadow` — deliberately weaker than its live `enforce`, because `off` (gate not consulted) is the dangerous state and recording the weaker minimum surfaces the true posture under `stronger_than_expected` rather than asserting a posture nobody signed off. The coverage test now derives from `HOLD_REASONS` itself, with `HOLD_REASONS_WITHOUT_MODE` naming the one genuine exemption (`kill_switch` is the binary cross-process stop, not a ladder) and a second test proving that exemption list cannot be used to hide a real mode config.

**`stronger_than_expected` is rendered, not just computed.** It appears in `main()`'s output and in the notifier's email rows. A note nobody sees never prompts anyone to update `EXPECT_MIN_MODE`, so the literal drifts further from live — which is how this class of bug starts.

**The ENFORCE-HOLD advice was itself a cry-wolf generator.** It told the operator to revert the offending gate to `shadow` if the hold looked spurious — which, under the new one-directional check, produces a weaker-than-intended ATTENTION every week until the revert is undone. It now says to lower that gate's `EXPECT_MIN_MODE` entry in the same change, so a deliberate revert is recorded rather than re-raised.
...
2. **`build_folds(data_end)` — three frozen folds plus one rolling.** The historical three stay byte-frozen so the version-over-version series remains comparable; a fourth fold tests the last `ROLLING_TEST_WINDOW_DAYS` (120) of the dataset — **both ends derived from the data**, `train_end = data_end - 120d`. Pinning the rolling fold's TRAIN end (the first cut did) reproduces the original defect on a slower clock: the fit ages while the test window grows without bound, so by 2027 it scores a year-stale model against a year of unseen regime. The aggregate gate inputs (`wf_auc_mean` / `wf_auc_min` / `brier_score`) are computed from the FIXED FOLDS ONLY, in both the pickle and the DB row, so they still reproduce v35..v42 exactly; the rolling fold is carried separately as `rolling_log_loss`. It is gated on log loss rather than macro-F1 because a 120-day window may legitimately contain no RISK_OFF day at all (measured: classes [1,2] only), which makes macro-F1 swing on class presence while log loss stays well defined against a fixed `labels=[0,1,2]`.
...
**Result.** Gate PASSES: fixed folds `0.906 / 0.963 / 1.000` giving `f1_min 0.9062` and `log_loss mean 0.0569` — **identical to the v35..v42 rows**, so comparability is preserved and no promotion is blocked — plus a rolling fold (`2026-05-07 → 2026-09-04`, n=82) at macro-F1 1.000 / log loss ~0.0000. The next scheduled weekly retrain (Fri 17:30) picks this up with no manual promotion; **no model was promoted as part of this change.**

5. **Staleness is now a GATE FAILURE, and the retrain heals its own data.** The rolling fold derives from the DATA's end, so a dataset that stops advancing yields a rolling fold that also stops advancing — the un-failable gate returning one level down. `regime_gate` therefore compares `requested_end` (calendar-anchored) against `train_end` (the data's real last day) and fails beyond `MAX_DATA_LAG_DAYS = 10`. And because a gate that fails every week is an outage rather than a fix, `_retrain_regime` now calls `extend_backfill()` before training: **nothing had ever scheduled `backfill_regime_snapshots.py`**, which is why the dataset froze in the first place.
...
**Hardened across six review rounds, and the reviews mattered.** The first cut fixed the four defects above and introduced or missed a dozen more; each round is recorded in the commit log. The ones worth remembering because they were *worse than what they replaced*:

- **The staleness guard armed a latent mislabeller.** `label_regime_day` coalesced with `row.get(k) or default`, which is NaN-blind (`NaN or 1.0` is NaN, and every later comparison against it is False). Harmless while the feed always produced a number; a live mislabeller the moment `_vix3m_as_of` correctly started returning None.
- **The term-structure ratio divided two different days.** Resolving numerator and denominator through independent "within 5 days" lookups is not an edge case but the DEFAULT live path — yfinance's ^VIX3M is ~95% NaN, so the denominator comes from FRED a business day behind the yfinance numerator. VIX 15→25 against a prior-day VIX3M of 16 reads 1.56 instead of ~1.32, across the 1.05 threshold, flipping the label to RISK_OFF on exactly the days the feature must be trusted. Now paired by date, as the crash governor always was.
- **The "de-risk on outage" guard sized UP.** It routed a dead feed to `_legacy_fallback`, whose `UNKNOWN` label maps to `regime_sizing_unknown = 1.0` — FULL SIZE. That default is right for its own case (no model yet, do not penalise early paper days) and exactly wrong for a data outage. A `_degraded_fallback` now returns RISK_CAUTION so an outage de-risks.
- **The remediation could have gone green over a hole.** A `today-30` backfill window cannot close a longer gap: it writes the recent tail, leaves the older hole forever, and drags `max(snapshot_date)` to today so the staleness gate reads CURRENT and passes. A false green is worse than the stale red it replaced. `start=None` now resumes from the last existing row.
- **A test read a gitignored file.** It patched a helper the shipping path no longer called, so the stub was a no-op and it read the developer's `macro_history.parquet`. Green locally, red on CI. The suite is now verified green both with and without that file.

**The meta-lesson.** Every one of those was a *repair* that looked correct and was not, in the same subsystem and the same session as the original finding. Fixing a silent-failure bug is itself an excellent way to introduce one, because the new code runs on the paths nobody watches. The reviews were not a formality here; they caught two live-behaviour regressions and one CI-red.
...
**Finding 1 — the reboot recovers nothing on its own.** There is no auto-start for MrTrader (no Task Scheduler entry, no Startup folder item, no Run key), and Postgres+Redis run under **Docker Desktop**, which launches from the `HKCU` Run key and therefore requires an *interactive logon*. An unattended reboot leaves the host at the lock screen with no database, no queue, and no app — a scheduled task alone would not have fixed it. Reboot events: 2026-07-25 00:31 and 2026-08-05 00:29, both Windows Update (`MoUsoCoreWorker.exe`) — a ~11-day cadence that a 2-week absence would very likely intersect.

**Finding 2 — a live reconciliation break was primed to freeze the book.** The DB ACTIVE DBC row still carried the original 2026-07-20 fill (257) and had never applied the two later fills (+7 on 07-27, −23 on 08-03); broker truth was 241. With `pm.reconciliation_mode=enforce` **and** `pm.kill_switch_sm_mode=enforce`, `reconcile_and_alert` calls `kill_switch_sm.on_reconciliation_fail()` → **HALT_NEW_RISK**, and de-escalation is **human-only**. The 2026-08-03 warning carried no `(shadow: not blocking)` suffix, confirming it was already enforcing; it survived only because the state machine is in-memory and the reboot cleared it. The next Monday cash-sleeve run (09:50 ET) would have re-latched it with nobody present to clear it.

**Decision**:
1. **Repaired the trade book to broker truth** (`scripts/repair_trade_book_20260805.py`, dry-run + JSON backup of every prior value). `id=144` DBC 257→241; `id=143` DBC stale `PENDING_FILL`→`CANCELLED`; `id=136` SGOV `PENDING_FILL`→`ACTIVE` (filled 06-22); `id=127` HPE 2-month-old ghost→`FORCE_CLOSED_NO_POSITION`. Read-only reconcile now returns **MATCH** on all 8 positions.
2. **Collapsed the phantom pending bands.** `reconcile()` treats pending as a *tolerance band* `[held, held+pend]`, so a stale pending row silently WIDENS what counts as acceptable. SGOV sat at held=0/pend=487 — band `[0, 487]` — meaning **a total liquidation of the cash sleeve would not have been flagged**. Promoting it to ACTIVE collapses the band to an exact point check. Post-repair `pending` is empty.
3. **Added a daily liveness beacon** (`daily_alive`, 07:45 ET, `day_of_week="0-6"`). Every probe is individually guarded so a failing sub-check *degrades* the report instead of suppressing it — a beacon that its own errors can silence would reintroduce the failure mode it exists to remove. It calls the pure `reconcile()`, never `reconcile_and_alert(mode=enforce)`, so the beacon itself can never escalate the kill switch.

**Rationale**: the owner elected to **pause Windows Updates** (already set through 2026-09-02, covering the trip) rather than enable auto-logon, which would store the account password as an LSA secret. That removes the reboot *trigger* but not the *consequence*, so the beacon matters independently: it converts "host silently dead" — the one failure the on-box dead-man watchdog structurally cannot report, since it dies alongside the brain — into an absence the owner can notice remotely. Weekends are included deliberately: a beacon legitimately silent on Saturday cannot detect a host that died on Friday night.

**Consequences**: the Aug 10 freeze is defused and the book reconciles exactly. Reboot risk is mitigated by the update pause but **not eliminated** — a power cut still leaves the stack down until someone logs in, and the accepted exposure of an outage is *missed trend rebalances* (staying long if the signal flips risk-off), not runaway loss: there are **0 resting orders at the broker** and every trend position carries `stop_price=None`, so no stop goes unenforced. Durable follow-ups for after the trip: a real auto-start path (run Postgres/Redis as native services or Docker-as-service so no interactive logon is needed), and setting `MRTRADER_SNITCH_URL` — the off-box dead-man's-snitch plumbing already exists in `heartbeat.ping_snitch()` and is one env var away from giving genuinely external detection. Not a WF/CPCV pipeline change, so `PIPELINE_ARCHITECTURE.md` is intentionally untouched.
...
**Findings → decision (fix the gateway-independent ones now; defer the connection-lifecycle ones to Phase 3 where a live gateway can validate them):**
- **G1 (HIGH) FIXED** — the cash-first canary would FAIL_CLOSED every rebalance: each sleeve reconciled the WHOLE DB book mis-tagged to its own venue → cross-venue phantom breaks (+ kill-switch escalation in enforce). Fix: `_db_signed_by_status` scopes the expected book to the venue that owns each row (via the sleeve's `resolve_venue`), so a split-venue book reconciles per-venue. No `Trade.venue` schema column (there's no migration infra) — config-derived, byte-identical while all sleeves are Alpaca.
- **G2 (HIGH) FIXED** — `startup_reconciler` (Alpaca-only, every 15 min) would ghost-CLOSE the IBKR-held cash sleeve within ~20 min → DB corruption. Fix: skip the ghost pass for a trend/cash trade whose sleeve is on a non-Alpaca venue. Byte-identical pre-cutover.
- **G3/G6 (HIGH/MED fail-OPEN) PARTIALLY mitigated** — a connected-but-unsynced IBKR session returns an empty `ib.portfolio()` → read as "flat" → full re-buy; and `NetLiquidation or 0.0` masked a missing NAV. Fix: `IBKRReadOnlyAdapter.get_positions`/`get_account` fail CLOSED on the BOTH-empty case (empty portfolio AND empty accountValues) + a missing NetLiquidation. **NOT fully closed (Opus review H2): accountValues stream before portfolio, so a values-synced-but-positions-not window is still fail-OPEN → Phase 3 must complete it with `accountDownloadEnd`/`positionEnd` + validate on a live gateway.**
- **G5 (MED→HIGH-at-cutover) FIXED** — the enforce whole-book gate (and per-name once enforced) fell OPEN on an INTERNAL eval error: its fail-safe returns allow=True and the caller only checked `.allow`. Fix: in enforce, HOLD on a set `verdict.error` too (matching reconciliation). **Caught a would-be regression in the paired venue-arg parity change**: passing the router's lower-case `'alpaca'` into the gate misses the case-sensitive `im.lookup` → false `unmapped` breach → false HOLD on the CURRENT enforce book; fixed by normalizing the gate's venue to the im-constant (uppercase).
- **DEFERRED to Phase 3 (gateway-gated, documented in the R1 plan + in-code TODO):** G4 distinct clientId (shared `ibkr.client_id=1` across 4 consumers → concurrent-connect collision), the IBKR read-adapter disconnect lifecycle, and the multi-asset gross-cap (trend inline gross-cap understates futures `market_value`). Plus 2 runtime notes: the readiness endpoint is `/api/dashboard/ibkr/readiness` (doc-drift fixed), and the live daemon (up since 07-25) must be restarted onto current `main` before any flip.

**Consequences**: zero live change (all fixes inert while venues are Alpaca; the gate normalization is byte-identical). +18 tests; full suite green; flake8 clean. Independent Opus review of the batch. **Known limitation (recorded here):** the reconciler provides ZERO cash-drift protection today — all live callers pass only positions (no `expected_cash`/`actual_cash`), so a cash mismatch is never a break (cash is not modelled DB-side; positions are the #1 phantom risk). The Phase-3 cutover checklist now carries the deferred blockers as hard gates.
...
**Q1 — trend by regime (confirmatory):** BULL Sharpe **+4.11** (maxDD −3%, hit 62%), NEUTRAL **+0.44** (maxDD −45%), BEAR **−0.77** (maxDD −47%, hit 43%). The regime structure is REAL and stark (trend needs the bull regime; it bleeds with deep drawdowns in neutral/bear). Combined with CH2 (conditional gross sizing on these regimes does NOT beat static), the structure is real but not tradeable via reactive gross sizing.

**Q2 — are the parked collinear strategies CONDITIONAL diversifiers? → NO, both CLOSED (the "rescue a parked keeper" question is answered: there is no CH4 candidate from them).** Per-regime corr-to-trend + the parked strategy's own standalone profile IN the decorrelating regime, over the 2008–2026 common window (4639 days). **A candidate must decorrelate AND stay active + non-losing there** — the guard (added after the CH3 review) against "uncorrelated-because-dead":
| parked | uncond corr | BEAR corr (n=1218) | BEAR standalone (ann_ret / Sharpe) | verdict |
|---|---|---|---|---|
| sector_rotation | 0.402 | **0.142** | **−34.2% / −1.26** | decorrelates but LOSES → NOT a candidate |
| credit_timing | 0.607 | 0.334 | −28.0% / −1.72 | collinear (0.33 > 0.30) + loses → CLOSED |

**Decision: NEITHER parked strategy is a conditional diversifier — both CLOSED.** `sector_rotation`'s correlation to trend DOES collapse to 0.14 in BEAR (n=1218, incl GFC), but it is NOT flat there — it is **active and LOSES −34%/yr** (Sharpe −1.26). So its low bear correlation is because it makes a DIFFERENT losing bet (long-biased sector rotation gets crushed in bear), not because it hedges. A strategy that loses 34%/yr in the regime where it "decorrelates" is a different way to lose, NOT a diversifier. `credit_timing` stays collinear (0.33) AND loses. **The naive "corr < 0.30 → candidate" reading (in the first CH3 draft) was WRONG — decorrelation alone is mechanically confounded for long-flat strategies; the standalone-value check flips the verdict.** The program's "could a regime-gated parked strategy rescue a keeper?" question is answered: NO.

**Q3 — does the live plain VIX crash governor earn its keep? → YES, on its drawdown mandate. KEEP it (this resolves the CH2c flag).** CH2c flagged that the governor doesn't beat constant-gross on trend-book Sharpe (−0.0024) and worsens the BEAR-regime Sharpe. CH3 evaluates it on the RIGHT objective (drawdown / crisis tail, via `evaluate_overlay`): **verdict "HELPS (shallower tail, Sharpe preserved)"** — full-sample max-drawdown **−1.8pp shallower** at a −0.003 Sharpe cost (within tolerance), and it reduces drawdown in every crisis (COVID **−4.25%**, BEAR_2022 −1.3%, GFC −0.4%) and in the BEAR regime (**maxDD −5pp shallower**). **Decision: KEEP the live plain crash governor as-is.** The small Sharpe drag CH2c found is the EXPECTED insurance premium — the governor delivers its actual mandate (crisis/bear drawdown reduction). **Nuance (noted, not acted on):** the governor is mildly counterproductive in NEUTRAL (maxDD −3pp *deeper*, Sharpe −0.13) because it cuts on non-crisis whipsaw-stress there — exactly CH2c's "trending-vs-whipsaw" insight — BUT CH2c's whipsaw-aware fix did NOT clear the gate (only +0.0006 vs static), so the net-positive plain governor stays unchanged.

**Consequences**: CH3 done, no live change. **No CH4 candidate came from the parked strategies** (both closed), so CH4 reverts to the originally-named **ranging-market MR sleeve** (the single pre-registered search, bound by the 12-month moratorium if it fails). The live crash governor is confirmed a keeper (CH2c flag resolved). NEXT = CH4 → CH5 (accrual). **Process note:** the first CH3 draft nominated sector_rotation as a CH4 candidate on correlation alone; the Opus review caught that decorrelation is mechanically confounded for long-flat strategies, so I added a standalone-value check (must be active + non-losing in the decorrelating regime) — which correctly reversed the verdict (sector_rotation loses 34%/yr in bear). This prevented CH4 being wasted on a strategy that "diversifies" only by losing differently. The compound-and-harden thesis holds: the edge is trend; the parked strategies don't diversify it in any regime; the governor is worth its premium.
...
## 2026-07-08 (CH2a + CH2 CONCLUSION — Compound-and-Harden) — trend-strength/breadth gross sizing decisively FAILS → KILLED; and with it, ALL THREE CH2 sizing multipliers are killed: conditional gross sizing does NOT beat constant-gross → keep the trend book's sizing SIMPLE

**Context**: CH2a is the last CH2 candidate — size gross by trend strength: lever UP when trends are broad + clean, cut when weak/conflicting (the premise: "weak-trend regimes are where TSMOM bleeds — cut there"). Operationalized TWO ways (so it isn't a one-measure strawman): trend **clarity** (mean over names of |ensemble-sign| = within-name lookback agreement) and trend **breadth** (fraction of the universe net-trending). 4 pre-registered configs (clarity primary/gentle/cut-only + breadth), gated through the shared harness (which handles m>1 up-sizing) vs constant-gross.

**Result — decisive KILL across BOTH measures:**
| config | signal | mean_sharpe | Δ vs base | BEAR |
|---|---|---|---|---|
| ch2a_primary | clarity, lever 0.5–1.5 | 0.5067 | **−0.1942** | −0.98 |
| ch2a_gentle | clarity, 0.75–1.25 | 0.5930 | −0.1079 | −0.894 |
| ch2a_cut_only | clarity, 0.5–1.0 (no lever) | 0.5620 | −0.1389 | −0.926 |
| ch2a_breadth | breadth, 0.5–1.5 | 0.6308 | −0.0701 | −1.074 |

**Decision: KILL CH2a — ship nothing.** Every config is far WORSE than constant-gross (−0.07 to −0.19), paired-improvement p=1.0 (governed never beats base). **Mechanism (sharpened by the Opus review — a cleaner finding than "contrarian"):** the correlation of the multiplier with the forward book return is ≈ **0** (−0.029 clarity / +0.009 breadth) — trend clarity/breadth carry **no book-level forward-Sharpe timing information**. Conditioning gross on a zero-edge signal can only INJECT NOISE: a fluctuating m uncorrelated with returns mechanically lowers Sharpe by a multiplicative-noise penalty ∝ Var(m), and indeed the SR loss is **monotonic in the dispersion of m** across all four configs (primary widest m-range → worst; breadth narrowest → least-bad). Decomposed: ~82% of the −0.19 is this noise penalty, ~18% is turnover (|Δm|·2bps ≈ 0.4%/yr, minor). The premise "weak-trend regimes are where TSMOM bleeds — cut there" is refuted: the cut-only variant (never levers) still loses −0.139, so cutting weak-trend periods hurts by itself. Registered `ch2a_trend_strength_gross` KILLED (N_TRIALS 28→29). *(Not tested: the clarity×breadth INTERACTION and ensemble-magnitude strength — a product of two ≈0-forward-corr signals is very unlikely to acquire timing edge, so omitting it is defensible, not fished; named here for honesty.)*

**★ CH2 PHASE CONCLUSION — antifragile sizing on the available trend-quality signals does NOT beat constant-gross; keep the trend book SIMPLE.** All three pre-registered CH2 multipliers are killed: CH2a trend-strength/breadth (−0.07 to −0.19), CH2b correlation-regime (−0.040), CH2c whipsaw-governor (+0.0006, n.s.). **Scoped claim (per the review — do NOT over-generalize):** conditional gross sizing on the *antifragility signals available to the trend book* (held-book correlation, VIX-term whipsaw, trend-strength/breadth) does not beat constant-gross OOS — because none of these signals has measurable book-level forward-Sharpe edge (corr ≈ 0), so conditioning gross on them only injects a Var(m) noise penalty. This does **NOT** mean "no conditional sizing can ever help": vol-*targeting* (`TSMOMConfig.book_vol_target`, scaling to a vol target rather than a trend-quality signal) is a DIFFERENT mechanism, untested here. **This is exactly the outcome the program's governing rule anticipated ("if it doesn't beat static, ship nothing — you've only added knobs"), and it is a valuable POSITIVE result: the constant-gross trend book's sizing is already near-optimal on realized history; reactive trend-quality sizing knobs degrade it.** The "make trend antifragile via trend-quality sizing" thesis is empirically retired. What remains from CH2: (a) the frozen finding that the whipsaw signal beats the plain crash governor (CH2c) + (b) the CH3 flag that the live plain governor doesn't beat static on trend-book Sharpe (scoped — its mandate is portfolio tail-insurance). **No live change from any of CH2** (nothing passed its gate). NEXT = CH3 (the regime-conditional diagnostic — now with the concrete governor question), then CH4/CH5.

**Consequences**: CH2 complete, all three killed, live book unchanged. The reusable, hardened gate harness (`app/research/ch2_sizing.py`) remains for any future sizing idea. Direction shifts to CH3 (diagnostic) → CH4 (the one pre-registered MR search) → CH5 (accrual). The compound-and-harden program's core empirical finding stands reinforced: the edge is trend, and the honest move is to operate it simply, not to over-engineer its sizing.
...
**Decision**: flipped **`pm.reconciliation_mode` = `enforce`** and **`pm.whole_book_gate_mode` = `enforce`** (owner-present, 2026-07-06). Config-only (no code change), read **live per-rebalance** via `get_agent_config` inside `trend_sleeve`/`cash_sleeve` → no uvicorn restart; setting back to `shadow`/`off` reverts instantly on the next rebalance. **`pm.kill_switch_sm_mode` stays `shadow`** (its enforce is gated on the cash buffer-replenish reduce-only refinement, a separate follow-up); `pm.drawdown_ladder_enabled` stays off.

**Rationale / verification**: independent Opus pre-flight review = **GO** (no CRITICAL/MAJOR). Verified against the enforce branches: on a DB↔broker break (`reconciliation.py` → FAIL_CLOSED, incl. internal-error fail-safe) OR a risk-policy cap breach (`whole_book_gate.py` gross/net-beta/notional/unmapped-symbol; a gate *evaluation* error still fails-SAFE `allow=True`), the sleeve does an **early return BEFORE the order-placement loop** — no liquidation, no short, no partial order. "A missed rebalance, never a bad trade" holds. Modes read + normalized (`.strip().lower()`), so only exact `"enforce"` acts and it can't silently degrade to shadow. **Watch caveat (Opus MINOR-1):** the trend gate's *import-failure* path holds rather than allows (safe direction, but would wedge trend into holding every rebalance) — confirm on the first post-flip rebalance (07-07 09:45/09:50) that the gate EVALUATES, not silently holds. PROJECT_STATE 2026-07-06.
