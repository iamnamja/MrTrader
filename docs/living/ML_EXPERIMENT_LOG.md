...
> **2026-09-06 REGIME GATE RE-OPENED — the walk-forward could not fail; under it, a 4-month-frozen training set and a live VIX3M feed pinned to a 7-week-old close.** Not an experiment — a hardening/re-validation pass (allowed under the moratorium; no strategy, threshold, or allocation changed). **Four stacked defects, each hiding the next:** (1) `_FOLDS` hardcoded with its last test window ending 2026-04-30 → the WF re-scored the same three windows weekly, so **v35→v42 all recorded byte-identical `wf_auc_mean=0.9563 / wf_auc_min=0.9062 / brier=0.0569`** — the promotion gate was evaluated against a constant; (2) `load_dataset` filters `snapshot_trigger=="backfill"` and the backfill stopped **2026-05-07**, so the weekly retrain re-fit the same 2179 rows for four months (differing model MD5s = XGBoost nondeterminism, not new information) — the 113 daily `premarket`/`startup_catchup` snapshots were filtered out; (3) `train_end` recorded the REQUESTED `end` (`date.today()`) rather than the data's max, so every weekly row claimed data it did not have — **this is what hid (2)**; (4) `RegimeFeatureBuilder` had no FRED backstop for `^VIX3M` and no staleness bound, and `_slice_to_date(...).iloc[-1]` carries the last row forward regardless of age → `build(2026-09-04)` returned `vix_term_ratio=0.7074` off the **2026-07-17** close (FRED-backed truth: 14.53/17.61 = **0.8251**), live, feeding `label_regime_day` and the sizing model. (4) is the **same yfinance `^VIX3M` rot that #674 fixed for the crash governor** — patched in one consumer, not the other; it also blocked the naive fix for (2), since backfilling first would have written a feature discontinuity into the exact window being added. **Fixes:** `_vix3m_as_of()` (yfinance PRIMARY → macro_history/FRED fallback, both under a 5-calendar-day staleness bound; no fresh value → NULL, not stale — verified **bit-identical on 12 historical dates 2018–2025**, so purely additive); `build_folds(data_end)` = 3 frozen folds + 1 rolling (`2026-04-30 → data_end`), with a loud WARN when the rolling fold is missing or thin (a skipped rolling fold silently restores the un-failable gate); honest `train_end` + separate `requested_end` + a >7-day staleness WARN; regime snapshots backfilled 2026-05-08 → 2026-09-04 (86 new rows, **0 NULL `vix_term_ratio`**). **Result: GATE PASSES** — folds `0.906 / 0.963 / 1.000 / 1.000`, `f1_min 0.9062`, `log_loss mean 0.0427`; folds 1–3 reproduce the historical numbers exactly (0.9563 / 0.0569) so the version series stays comparable and **no promotion is blocked**. **No model promoted** — the next weekly retrain (Fri 17:30) picks it up. ⚠️ **Known limitation:** `label_regime_day` is a deterministic rule over features that are themselves model inputs, so the model is a rule-approximator and macro_F1→1.0 is the expected ceiling, NOT predictive skill. The rolling fold catches pipeline breakage / fitting failure / label-distribution shift; it does **not** catch loss of predictive edge, and would not by itself have caught defect (4) (a stale `vix_term_ratio` moves rule label and prediction together). Read a PASS as "the machine is intact", not "the regime signal still works". Suite 4593 passed / 8 skipped. DECISIONS 2026-09-06.

> **2026-07-22 UNIVERSE-EXPANSION STUDY — NO orthogonal-macro ETF improves the trend book → KEEP the 10-ETF universe. 0/12 PASS.** Owner-authorized re-validation of the EXISTING trend edge on a broader asset set (same TSMOM engine + same DUAL gate as CH2 → permitted under the moratorium, not a new-edge hunt). Pre-registered (`docs/reference/UNIVERSE_EXPANSION_PREREGISTRATION_2026-07-22.md`); harness `app/research/universe_expansion.py` → `docs/reference/universe_expansion_results.json`. Tested 8 orthogonal-macro ETFs (USO/UNG/DBA/FXE/FXY/VNQ/TIP/SLV) added individually + 4 groups through the SAME CPCV path CH0a was frozen on; **in-run baseline reproduced 0.7009 exactly.** **Result: 0/12 PASS — EVERY addition LOWERS CPCV mean_sharpe** (best = +DBA 0.6809, Δ−0.020; worst = kitchen-sink-18 0.4935, Δ−0.207), none beats 0.7009, none significant (all paired-bootstrap p 0.37–0.94). Mechanism: the 10-ETF basket already spans the macro factors; under a fixed gross budget + inverse-vol sizing, more names DILUTE concentration on the best trends and add lower-quality/higher-cost legs (USO/UNG roll drag, SLV noise + GLD-collinear, FX trends poorly). **Nuance (recorded, not a reversal):** several commodity adds IMPROVE the BEAR tail (USO Δbear +0.265, commodities-group +0.311) — real crisis-diversification — but the BULL/NEUTRAL give-up outweighs → still fail the primary. **Decision: KEEP `pm.trend_universe` as-is.** Confirms the CH verdict empirically. Also this session: covered-call ETFs + DIY covered-call writing REJECTED (packaged/naked short-vol; anti-trend; mechanically impossible at $100k). No live change. DECISIONS 2026-07-22.

> **2026-07-08 CH4 — the ONE pre-registered terminating search (ranging-MR sleeve) → KILL; the 12-month hunting MORATORIUM binds. Compound-and-Harden COMPLETE through CH4.** Short-horizon time-series mean-reversion on the 10-ETF universe, active ONLY in ranging regimes (low trend-clarity AND low vol; flat otherwise). Gates FROZEN before the run (`docs/reference/CH4_MR_PREREGISTRATION_2026-07-08.md`); decision on the pre-registered primary ONLY (no best-of-8). **`mr_primary` FAILS 3/5 gates:** Track-A (CPCV mean_sharpe 0.012 — no edge), off-regime Sharpe −0.11, **Track-B residual-α t −0.52** (anti-diversifying). The ranging regime is rare (2% active — low-clarity days come with high vol, so the intersection barely occurs). **The pre-registration caught a mirage:** the one good-looking config (`mr_loose_regime`, a sensitivity config: CPCV **0.82**, Track-A PASS, Track-B t **2.77**) **FAILS the detection-lag gate** — its CPCV mean_sharpe collapses to 0.36/0.16/0.58 under a 1/2/3-day *regime* lag (boundary-fit, not robustly harvestable); because the decision was on the primary, it couldn't flip the verdict. Opus-reviewed: KILL is real (the pipeline CAN pass — loose_regime cleared Track-A+B — so the primary null is genuine), lag-rejection sound. Registered `ch4_ranging_mr` KILLED (N_TRIALS 29→30). **12-month moratorium in effect to 2027-07-08; program is now CH5-only.** No live change. DECISIONS 2026-07-08 (CH4 RUN).

> **2026-07-08 CH3 — regime diagnostic: NEITHER parked strategy is a conditional diversifier (both CLOSED); the live crash governor EARNS its keep (resolves the CH2c flag).** Read-only (`scripts/ch3_regime_diagnostic.py` → `docs/reference/ch3_regime_diagnostic.json`); 3 questions by frozen BULL/NEUTRAL/BEAR labels. **Q1 (trend by regime, confirmatory):** BULL SR +4.11 (annRet +27.7%), NEUTRAL +0.44 (+4.3%), BEAR −0.77 (−8.0%) — structure real, not tradeable via sizing (CH2). **Q2 (conditional collinearity — corrected after the CH3 review):** a candidate must decorrelate AND stay active + non-losing in the decorrelating regime (guard vs 'uncorrelated-because-dead'). `sector_rotation` corr-to-trend collapses to 0.14 in BEAR (n=1218 incl GFC) BUT is active + **LOSES −34%/yr there (Sharpe −1.26)** → decorrelates by making a DIFFERENT losing bet, NOT a hedge → **NOT a candidate**. `credit_timing` collinear (0.33) + loses → CLOSED. **NO CH4 candidate from the parked strategies** — the "rescue a keeper" question is answered NO. (The first draft nominated sector_rotation on corr alone; the review caught that decorrelation is mechanically confounded for long-flat strategies; the standalone-value check reversed it.) **Q3 (governor tail, resolves CH2c):** on the RIGHT objective (`evaluate_overlay`), the plain VIX governor = **HELPS (shallower tail, Sharpe preserved)** — full maxDD −1.8pp shallower at −0.003 Sharpe; contiguous crisis dd_improve COVID −4.25% / BEAR_2022 −1.3% / GFC −0.4%. **KEEP the governor** — the CH2c Sharpe drag is the expected insurance premium. (Per-regime maxDD/Calmar are concatenation artifacts, caveated; the decision rests on full-sample + contiguous crises.) No live change. NEXT = CH4 (the ranging-MR sleeve — parked strategies gave no candidate). DECISIONS 2026-07-08 (CH3).

> **2026-07-08 CH2a + CH2 CONCLUSION — trend-strength/breadth gross sizing decisively KILLED; ALL 3 CH2 sizing multipliers killed → conditional sizing doesn't beat constant-gross, keep the trend book SIMPLE.** CH2a sizes gross by trend strength (lever up broad+clean trends, cut whipsaw), operationalized TWO ways (clarity = |ensemble-sign| agreement; breadth = fraction net-trending) so it's not a one-measure strawman. **4 pre-registered configs, all decisively FAIL:** clarity primary 0.5067 (Δ**−0.1942**, BEAR −0.98), gentle 0.5930 (−0.108), cut-only 0.5620 (−0.139), breadth 0.6308 (−0.070, BEAR −1.074); paired-improvement p=1.0. Mechanism (Opus-sharpened): corr(m, forward book return) ≈ 0 (−0.029 clarity / +0.009 breadth) — the signals carry NO forward-Sharpe timing edge, so conditioning gross on them only injects a multiplicative-noise penalty ∝ Var(m) (SR loss monotonic in m-dispersion; ~82% noise / ~18% turnover). Cut-only (no lever) still loses −0.139 → "weak-trend regimes bleed, cut there" is refuted. Registered `ch2a_trend_strength_gross` KILLED (N_TRIALS 28→29). **★ CH2 PHASE RESULT (scoped):** CH2a (−0.07/−0.19) + CH2b correlation (−0.040) + CH2c whipsaw (+0.0006 n.s.) → **conditional gross sizing on the available trend-quality signals (correlation/whipsaw/strength-breadth) FAILS the dual gate** (none has forward-Sharpe edge → Var(m) noise). Does NOT generalize to vol-TARGETING (`book_vol_target`, a different mechanism, untested). A valuable POSITIVE result: the constant-gross trend book's sizing is already near-optimal; reactive trend-quality knobs degrade it. Thesis retired; no live change (nothing passed). NEXT = CH3. DECISIONS 2026-07-08 (CH2a + CH2 conclusion).

> **2026-07-08 CH2c — whipsaw-aware crash governor → KILLED (beats the plain governor, not constant-gross); flags the live plain governor.** De-risk only when stressed (VIX/VIX3M backwardation) AND trends are choppy (whipsaw = 1 − trend clarity), so a broad TRENDING crash (2008/2022, where trend profits) is NOT cut. Gated vs constant-gross + the plain (stress-only) VIX governor as reference. **Plain VIX governor** (LIVE, default ON): mean_sharpe 0.6985 (Δ−0.0024), **BEAR −0.883** (much worse than ungoverned −0.769). **ch2c_primary**: 0.7015 (Δ+0.0006, p 0.56 n.s., BEAR −0.791), **beats the plain governor +0.003** (whipsaw-awareness works — recovers the trending-crash cut + the bear tail) but only +0.0006 vs constant-gross (n.s.) + still regresses BEAR → KILL. Registered `ch2c_whipsaw_governor` KILLED (N_TRIALS 27→28). **CH3 flag (scoped, not a verdict):** measured ONLY on the trend book's Sharpe + bear-regime Sharpe, the live plain crash governor doesn't beat static + worsens the bear tail — but this harness does NOT measure its actual mandate (portfolio drawdown/tail-insurance, F1b), which mechanically lowers Sharpe. A CH3 question on the right objective, not a live change here. Report-only. DECISIONS 2026-07-08 (CH2c).

> **2026-07-08 CH2b — correlation-regime gross scaling → KILLED (does not beat constant-gross).** First CH2 antifragile-sizing candidate + the shared gate harness (`app/research/ch2_sizing.py`: governed book = base × m[t] through the SAME `evaluate_sleeve`→CPCV path CH0a was frozen on; DUAL gate = beat mean_sharpe 0.7009 AND don't regress BEAR ≥ −0.77). CH2b cuts gross as the HELD-book weighted avg pairwise correlation rises (the book collapsing to one bet). Harness gates governed = base × m[t] through the SAME CPCV path CH0a was frozen on, TRIPLE gate (beat mean_sharpe SIGNIFICANTLY via paired bootstrap AND don't regress BEAR), harness-owned PIT lag + re-sizing-turnover charge + full-precision base (m≡1 reproduces 0.7009 exactly). **3 pre-registered configs, all FAIL every prong:** primary (w63, 0.60→0.90, floor 0.50) mean_sharpe **0.6607 (Δ−0.0402, improve p 0.68 n.s., BEAR −0.795 regresses)**, shorter-window 0.6591 (BEAR −0.829), gentler-band 0.6825 (Δ−0.0183). Mechanism: high held-book correlation is predominantly a strong equity-BULL signal (trend's best periods), not a crisis signal → cutting gross there sacrifices return; correlation is direction-blind. KILL, ship nothing (registered `ch2b_correlation_gross`, N_TRIALS 26→27). Motivates CH2c (add the trend-direction signal). Report-only; no live change. DECISIONS 2026-07-08 (CH2b).

> **2026-07-07 CH0a — constant-gross ETF-trend BASELINE frozen (the CH2 out-of-sample bar).** Ran the ungoverned live trend book (`live_trend_book_returns()` = `tsmom_backtest(TSMOMConfig())`, all antifragile/governor multipliers = 1.0) through the uniform `evaluate_sleeve` → purged/embargoed CPCV → Ruler-v2 gate + a regime-conditional Sharpe breakdown. **Result (window 2007-01-04..2026-06-12, n_obs 3661, n_folds 8):** CPCV **mean_sharpe 0.7009**, path-t 5.4872, point_SR 0.7002, worst_regime_SR −1.657; standalone Sharpe 0.724 / maxDD −13.9% / Calmar 0.487; regime-conditional **BULL +4.11 (26% days) / NEUTRAL +0.44 (45%) / BEAR −0.77 (29%)**. Fold-coverage LOW (no stress/BEAR fold; n_folds 8 < CAPITAL floor 10) → **the CH2 gate is DUAL: beat mean_sharpe OOS AND not regress the BEAR regime-conditional Sharpe.** Frozen (pinned `end=2026-07-07` + data fingerprint) in `docs/reference/ch0_trend_baseline.json` (`scripts/ch0_baseline.py`); Opus-reviewed (no CRITICAL). This is a FROZEN benchmark, not a model change — live book unchanged. DECISIONS 2026-07-07 (CH0); SSOT `docs/reference/COMPOUND_AND_HARDEN_PROGRAM_2026-07-07.md`.

> **2026-07-05 CORRECTION (harness audit) — LX1 / LX9-A ran as 2-feature scorers, NOT the documented "5-feature IC-validated composite".** The 2026-07-05 harness-correctness audit (`docs/reference/HARNESS_CORRECTNESS_AUDIT_2026-07-05.md`, PIPELINE_ARCHITECTURE KL-12) found the fundamentals PIT filter in `factor_scorer.py:657` was a **dead branch** (wrong date-col name) → the 3 fundamental features (profit_margin, operating_margin, pe_ratio) were **silently dropped** from every IC/LX scorer. So LX1/LX9-A were effectively **momentum + 52w-high (2-feature)** scorers; the intended 5-feature composite was never actually tested (`pe_ratio` isn't even in the parquet). **The KILL verdicts stand and are safe:** all affected runs were KILLs (zero false KEEPs), V219–V224 already clip fundamentals weights to 0.0, and the V221 pre-fold IC audit found all 3 fundamentals have **negative** IR (−0.16 / −0.21 / −0.004) → restoring them rescues nothing. The conclusion "ML weighting on this feature set is ruled out" is accurate **for a 2-feature scorer**; the 5-feature composite is a technically-untested (but low-prior) variant. An attribution-only re-run after the `factor_scorer` fix is optional bookkeeping — expectation: verdicts unchanged. Fix not yet landed (report-only audit).

> **2026-06-25 Audit-6 eval re-runs — L/S ranker 0/4 PASS (thesis stays DEAD); intraday WF BLOCKED.** Closing the audit's "re-run before relying on these evals" loop after the 6a/6b research-sim fixes. **L/S research WF** (`run_ls_research_walkforward`, Russell-1000, 5-fold/6yr, with the corrected signal-mode short-margin from Wave 6a): **0/4 scorers passed** — A_QualityShort avg −0.59 (min −1.20), B_MeanReversionShort −0.41 (−1.36), C_SectorRelative −0.39 (−1.24), D_Combined +0.73 but min-fold −0.36 (fails the no-fold<−0.30 guard). The 6a margin fix does NOT resurrect the ranker; the dollar-neutral L/S thesis stays **DEAD**. Live book is long-only → unaffected. (NB: run had no `--as-of` → fold boundaries anchored to today; re-run with `--as-of` for reproducibility.) **Intraday WF/CPCV (6b): NOT runnable / SKIPPED** — the DB-active intraday model (v63) won't load (BUG-13 guard: `.gate_passed` sentinel missing; only old v29 has one), and a proper re-run needs a full intraday retrain (overnight). Intraday is dead (per-fold CPCV −2.80) and OFF → not worth a retrain; skipped, NOT bypassed (won't touch a safety sentinel to force it). **6c** (`wf_auc` time-order) is report-only (no gate impact); **6d** (cache split/div basis) auto-corrects on a fresh cache → no discrete re-run. **Net: every live position + KILL verdict remains trustworthy; no eval re-run changes a decision.** Report-only.

> **2026-06-22 Option A — sector-ETF rotation swing sleeve → REAL standalone but REDUNDANT to trend (Track-B FAIL).** First real application of the Phase A combined-book evaluator. `app/research/etf_rotation.py` + `@register_sleeve("sector_rotation")`: cross-sectional **12-1 relative-strength rotation**, top-K=4 of 11 SPDR sectors, monthly, inverse-vol (vol-floored), Antonacci dual-momentum cash filter — DISTINCT from the live sleeve's absolute per-asset TSMOM. Pre-registered/standard config, NOT swept. **Verdict (18.5y): STANDALONE PAPER-PASS** (CPCV mean Sharpe **0.86**, path-t 6.67, HAC p 0.0006) **but TRACK-B vs the live trend book FAILS** (corr **0.51**, dSR +0.07, P(ΔSR>0) 0.875 < bar, tail-overlap) → **redundant, not book-additive → PARKED** (family_registry N_TRIALS 25→26). It's the trend edge re-expressed, not a diversifier. A genuinely additive swing book needs a lower-corr source (single-stock momentum on survivorship-free data = Option B / Norgate). Independent deep-dive: **no look-ahead** (price-perturbation injection — past byte-invariant to future); MAJOR zero-vol weight blow-up fixed (vol floor) + tests added. `scripts/run_sector_rotation_eval.py` + 9 tests; full suite 3864 green; flake8 clean; report-only. DECISIONS 2026-06-22 (Option A).

> **2026-06-22 Phase A — unified combined-book multi-strategy WF evaluator (holistic eval is now one call).** `app/research/multistrat_eval.py` (`run_multistrat_eval`) evaluates a SET of strategies AS ONE BOOK by reusing the existing machinery: assemble the book (`build_book`, PIT inverse-vol) → run the book RETURN SERIES through the SAME CPCV the single-sleeve gate uses (`SeriesReturnStrategy`+`run_cpcv`) → book-level mean Sharpe / path-t / %positive / worst-path + a **family-level deflated Sharpe** (P0.5 count); optionally apply the risk-policy-v1 drawdown de-gross LADDER (PIT) and re-run; per-sleeve standalone SR + avg weight + **leave-one-out Track-B**; GL-1 cross-strategy tail; and a fold-in **union** book for ragged history (common-window = the CPCV verdict). Adding a strategy is now a one-call holistic check. Two deep-dive passes (1 self + 1 independent): look-ahead-free (governor + union strictly PIT; book series feeds CPCV without double-shift) and DSR-argument-correct; review fixes applied (kwargs guard, union cost parity, warmup trim, strict governor test). `scripts/run_multistrat_eval.py` + 11 tests; full suite 3855 green; flake8 clean; report-only (no live path). Phase B (ERC/covariance sizing) + Phase C (research↔live replay parity) remain data-gated (= R2/IBKR). SSOT `docs/reference/HOLISTIC_MULTISTRAT_STATE_AND_SCOPE_2026-06-22.md`; DECISIONS 2026-06-22 (Phase A).

> **2026-06-22 Alpha-v10 P0.5 — family-level trial counting: the program's true N_TRIALS is now AUDITABLE (25, not the hardcoded ~20).** The panel's point: "rules-based sleeves are OOS-by-construction" is false at the family-selection level — we've searched ~two dozen families, an uncounted multiple-testing burden. `app/research/family_registry.py` enumerates **every distinct strategy FAMILY searched** (25 trial families; 27 entries with cash + futures-book ensemble explicitly excluded for auditability): **LIVE 3 / PAPER 5 / KILLED 14 / PARKED 3 / SCAFFOLD 2**, each with status + verdict + doc ref, plus a research degrees-of-freedom log (variants/reruns/post-hoc exclusions). `family_trial_count()` (=25) now feeds `null_zoo`'s parametric Deflated-Sharpe cross-check (`dsr_family`/`n_families`; dsr_n10/30 kept as sensitivity), replacing the guess. The enumerated 25 > 20 → the deflation is slightly MORE conservative (`deflated_sharpe` is monotone-decreasing in N, verified). **GL-0 verdict UNCHANGED** — the verdict is decided only by the empirical max-stat p-values (the DSR is a notes-only cross-check, never a hard veto), so 20→25 cannot flip it; DSR stays borderline (<0.95) → confirms "size modestly." Independent review verified the two integrity properties (verdict independent of DSR; deflation monotone in N), no blocker. `scripts/run_family_registry.py` + 11 tests; full suite 3843 green; flake8 clean; report-only. **P0 (trust-the-numbers) now COMPLETE.** SSOT `docs/reference/P0_5_FAMILY_REGISTRY_2026-06-22.md`; DECISIONS 2026-06-22 (P0.5).

> **2026-06-22 Alpha-v10 P0.4 — credit de-risk overlay PIT re-verdict → PIT-ROBUST (panel "shrinks under PIT vol" hypothesis REFUTED).** The Alpha-v10 panel bundled the credit overlay with carry under "kill in-sample vol-matching → the +0.064 likely shrinks." P0.2 confirmed that for carry (in-sample +0.17 dSR → ~0 under PIT). P0.4 shows the credit overlay is **structurally different**: it is a scale-invariant time-varying de-risk MULTIPLIER (`m[t]·base[t]`) measured on an already-PIT trend book — its dSharpe never depended on vol-matching. Re-running the G1 confirmatory harness (`run_credit_curve`) on current data **reproduces the number**: marginal-vs-VIX-governor **dSharpe +0.0639** (≈ original +0.064), **dCalmar +0.0298** (≈ +0.030), helps in all 3 crises (GFC +0.019, COVID +0.022, BEAR +0.006), both-halves overall-dMaxDD positive (H1 +0.022, H2 +0.001). It did NOT shrink. **Status UNCHANGED: marginal tail-insurance CANDIDATE, flag OFF** (`pm.credit_governor_enabled` default false) — not killed (real PIT-robust tail hedge), not promoted (small + post-hoc trigger). Binding caveat is **multiplicity** (L=120/band=0.02 is post-hoc; the pre-registered L=60/band=0 failed), NOT vol. (`curve_governor` unchanged: NO-TAIL-BENEFIT, marginal dSharpe −0.018, stays off.) `scripts/run_credit_pit_reverdict.py` (reproducer + verdict fn); 5 tests; full suite 3821 green; flake8 clean; report-only (no live-path change). Independent adversarial review upheld the no-vol-artifact claim on all axes. SSOT `docs/reference/P0_4_CREDIT_PIT_REVERDICT_2026-06-22.md`; DECISIONS 2026-06-22 (P0.4).

> **2026-06-20 Alpha-v10 P3.1 — VRP via the VIX-futures curve: a real premium → REVERSES the earlier VRP park.** Cheap owned-data re-test of the wrongly-parked VRP (was judged as alpha on short options data). `app/research/vix_vrp.py`: short the front VIX future (Norgate VX) in CONTANGO (roll-down), flat in backwardation, gated by the crash-governor signal (VIX<VIX3M); registered `vix_vrp` (P3-VIX-VRP). **Sharpe 0.64** (pre-2015 0.93, post-2015 0.53, 2020s 0.51), HAC p 0.0018, maxDD −18%; official **Track-A PAPER-PASS** (point_SR 0.47). **Survives the vol crashes** (the make-or-break test): Feb-2018 −4.4%, COVID-2020 −4.8% (the gate flips flat on backwardation — a naive short-vol loses 50-90%). Marginal diversifier (corr-to-trend 0.46, residual-α t 1.46). **Caveat:** CPCV flagged "no stress-regime fold" → crash-survival is the manual event-study, not the CPCV; re-confirm live. A 4th real risk-premium sleeve (trend, carry, xsmom, VIX-VRP) — short-vol, the opposite crash-convexity to trend. PAPER-candidate. 3 tests; 229 regression green; flake8 clean; report-only. See DECISIONS 2026-06-20 (P3.1).

> **2026-06-20 Alpha-v10 P1.4 — more free factors: basis-momentum + CFTC CoT both KILLED; the free zoo is exhausted at carry + XS-momentum.** Basis-momentum (12m cumret front − 2nd-nearby, long high; built the 2nd-nearby return as rank-2's own pct_change): Sharpe −0.10, residual-α t 0.47 — orthogonal (−0.20) but no edge; academic "differentiated edge" didn't replicate. CFTC CoT hedging-pressure (verified Socrata API + mapped 37 markets to contract codes, release-lagged PIT, long high net-spec/OI): full-sample Sharpe +0.06 (doesn't clear floor); **perfectly orthogonal (corr 0.03/0.01 to carry/xsmom — the panel was right)** but residual-α t 0.27 → no edge. Both KILLED at the pre-registered sign (no flipping). CoT's recent sub-period IMPROVES (2010s 0.14 → post-2015 0.42 → 2020s 0.77) — noted as a possible regime-conditioned revisit, NOT acted on (cherry-pick trap); CFTC code mapping preserved in ALPHA_V10 §P1. **Six free factors now tested → only XS-momentum survived; the free zoo is exhausted at carry + XS-momentum** (lesson reaffirmed: orthogonality without standalone return doesn't help). Next real progress = P2 IBKR execution-truth. Report-only; no live change; no production modules for the dead factors. See DECISIONS 2026-06-20 (P1.4).

> **2026-06-20 Alpha-v10 P1.3 — futures multi-factor book: a SIGNIFICANT diversifier (the second engine).** "Gate the basket": do the two individually-MARGINAL factors (carry t~1.76, xsmom t~1.6) combine to clear a bar neither did alone? Registered `futures_book` (P1-3-FUT-BOOK) = equal-weight(futures_carry, futures_xsmom) — both already vol-targeted + roll-costed. **YES:** book Sharpe 0.67 (2010s 0.92, post-2015 0.83, 2020s 0.65), corr(carry,xsmom) 0.42; official **Track-A PAPER-PASS (mean_sharpe 0.84, point_SR 0.85)** — strongest sleeve yet. **Track-B vs live ETF-trend: residual-α t 2.29 (SIGNIFICANT >1.96), resid-Sharpe 0.56, beta 0.24** (vs carry-alone t 1.76) — the composite's diversification crosses conventional significance. The genuine "second engine" to pair with live trend (still paper; needs P2 IBKR execution-truth before capital). 6 tests; flake8 clean; report-only. Phase-1 free-data factor mining substantially complete; remaining: P1.1 CFTC CoT + basis-momentum/calendar-spread. See DECISIONS 2026-06-20 (P1.3).

> **2026-06-20 Alpha-v10 P1.2 — futures factor zoo: XS-momentum is a real new factor; curve-mom/value/skew KILLED.** The panel's highest-EV FREE move (mine the data we own). Reuse: `carry_backtest` is a generic cross-sectional engine. Built `app/research/futures_factors.py` (xs_momentum/curve_momentum/value/skewness signals + `xs_factor_backtest`) + `scripts/run_futures_factors.py`. All four tested on the 76-market universe (3bps/side roll). **Only xs_mom (cross-sectional 12-1) survives → registered `futures_xsmom` (P1-FUT-XSMOM): Sharpe 0.56, modern-robust (post-2015 0.58), corr-to-trend 0.12 (RELATIVE momentum, distinct from absolute trend), residual-α t +1.60; official Track-A PAPER-PASS (mean_sharpe 0.72, point_SR 0.72).** curve_momentum (−0.24) / value/5y-reversal (−0.24) / skewness (+0.03) KILLED at the pre-registered sign — NOT sign-flipped (OPT-5 trap). Now have carry + XS-momentum as the two real futures factors. 5 tests; 233 regression green; flake8 clean; report-only. **Next: P1.1 CFTC CoT + P1.3 ensemble.** See DECISIONS 2026-06-20 (P1.2).

> **2026-06-20 Alpha-v10 P0.3 — ruler negative controls: the gate is CLEAN (Type-I ~nominal).** Proving the panel's "is the ruler leaky?" worry is unfounded. Built `app/research/ruler_controls.py` (MC) + `scripts/run_ruler_controls.py`. **(A) True-null PAPER FP rate:** floor-alone 23.6% (the known point-SR-floor leak) → JOINT with the HAC floor **5.3%** (n=1500) / 5.3% (n=3000) — HAC closes it to nominal. **(B) Anti-correlated zero-edge null → Track-B residual-alpha pass-rate 5.7%** (~size) — NOT gamed by anti-correlation. → the gate's Type-I error is controlled on both tiers; PASS/FAIL trustworthy at face value. Retroactively validates the P0.2 switch to residual-alpha Track-B (the vol-matched dSR would NOT have cleared these controls). 3 tests; flake8 clean; report-only. See DECISIONS 2026-06-20 (P0.3).

> **2026-06-20 Alpha-v10 P0.2 — carry honesty pass: real but recalibrated down (Sharpe 0.66→0.58; diversification MARGINAL).** Answering the panel's loudest criticism. Built `app/research/futures_roll.py` (roll-day schedule) + `roll_cost_bps` in `CarryConfig`/`carry_backtest` (3bps/side baked into the official sleeve + runner). Roll cost is TRANSACTION-only (round-trip 2×|W| per roll day) — does NOT subtract the roll yield (already in the back-adjusted returns) → no double-count. **Standalone Sharpe 0.66→0.58** @3bps (drag ~1.1%/yr; 0.63@1bps/0.52@5bps); still HAC p 0.0001, post-2015 0.81, 2020s 0.55; official **Track-A PAPER-PASS** (point_SR 0.71). **Diversification REAL but MARGINAL:** the old in-sample 50/50 vol-matched "+0.17 dSR" was an artifact (→ ~0.00 under PIT rolling-vol; panel was right); the budget-invariant **residual-alpha Track-B = +5.4%/yr, HAC t ~1.8 (not conv. significant), resid-Sharpe 0.43, beta-to-trend 0.31** → "probably helps the book," not a slam-dunk. Partly an energy/VIX bet (top contributors VX+energy) but survives their removal (ex-energy ~0.54). → carry stays a PAPER-candidate at the honest ~0.58; measured paper-deploy, not urgent capital. 11 new tests (roll schedule + roll-cost transaction-only/PIT); 187 regression tests green; flake8 clean; no live change. See DECISIONS 2026-06-20 (P0.2) + ALPHA_V10_SYNTHESIS_AND_PLAN §P0.

> **2026-06-19 Alpha-v9 P4-2c — rebalance-cadence study: keep WEEKLY for trend AND carry (daily-carry edge is NOT deployment-robust).** Question: why weekly not daily? Built a backward-compatible no-trade `rebalance_band` into the TSMOM + carry engines (default None = unchanged; live ETF-trend still 0.724, 281 regression tests green) + a pre-registered study (`scripts/run_rebalance_study.py`, `P4-2c-REBALANCE-CADENCE`). **TREND**: weekly +0.83 vs daily +0.50 / band +0.52 — daily/band far worse, modern era negative (whipsaw + turnover). **CARRY**: daily wins full-sample (+0.72 vs weekly +0.66) but ONLY via the early period — sub-periods weekly [2000s 0.70 / 2010s 0.97 / **post-2015 0.89** / **2020s 0.62**] vs daily [0.78 / 1.04 / **0.79** / **0.44**]: in the modern (deployment-relevant) regime WEEKLY BEATS DAILY; band underperforms (+0.45). Frozen criterion (beat weekly full-sample AND post-2015 AND no sub-period worse >0.15) → both keep weekly. The full-sample daily-carry number was a non-robust artifact (the sub-period guard caught it, à la F3 carry / P3-5). 6 band tests; flake8 clean; report-only, no live change. See DECISIONS 2026-06-19 (P4-2c).

> **2026-06-18 Alpha-v9 P4-2 HARDENING — 4-agent adversarial Opus review; carry edge SURVIVES; deploy carry not futures-trend.** Audited the whole futures pipeline (look-ahead/PIT, data-integrity, backtest-math, tests). Fixed real bugs (none overturned the verdict): negative-denominator sign-flip in `true_returns` (CL-2020 −37.63 prior price flipped a loss to a +23.5% gain inside the winsor band → `den.where(den>0)`); carry expiry now from the **scheduled contract code** not the realized last date (removed roll-time hindsight AND a 1-3-month **recent-carry staleness** bug — coverage now reaches 2026-06-17); carry cross-section guards (dead `std==0` guard → `>1e-8` + `min_xs_width=5` + `ffill(limit=10)` + cost on actual levered turnover); `pct_change` deprecation made explicit-pad (live ETF-trend unchanged at 0.724, crypto unchanged); universe micros/STIRs classified by NAME (fixes M2K/MHI escaping + the ann-vol cut wrongly removing real bonds; 73→76 markets). **Survivorship CRITICAL flag QUANTIFIED IMMATERIAL** (full 0.66 vs ≤2005-history 0.65; post-2015 0.89 both; only 6/76 markets recent). **Verdict CONFIRMED**: carry PAPER-PASS (mean_sharpe 0.79, point_SR 0.81), post-2015 +0.89, Track-B dSR +0.17. +6 regression tests that actually catch the bugs (real look-ahead guard w/ time-varying signal; varying-denominator; negative-denom drop; degenerate cross-section). 22 futures + 224 tsmom/crypto/sleeve tests green; flake8 clean. **Full-book run: add CARRY to the live ETF-trend book (0.72→0.89); do NOT add futures-trend (decayed + redundant, corr 0.44 → 0.57).** See DECISIONS 2026-06-18 (P4-2 hardening).

> **2026-06-18 Alpha-v9 P4-2 — futures CARRY is a real, MODERN, diversifying edge (first new deployable edge since trend); trend has DECAYED.** On the local Norgate mirror (survivorship-free, 73-market liquid universe), tested the two managed-futures factors free data couldn't: `app/research/futures_data.py` (correct difference-adjusted returns `ΔCCB/unadj_prev`, winsorized; liquid/micro/STIR filter), `futures_carry.py` (term-structure carry), sleeves `P4-2-FUT-TREND`/`P4-2-FUT-CARRY`, `scripts/run_futures_research.py`. **CARRY: standalone Sharpe 0.67, positive in EVERY sub-period incl. modern (2010-19 +1.00, post-2015 +0.84), signs economically correct (NG deep contango, energy backwardated), cost-robust (0.65 @2×). Official Ruler-v2 Track-A PAPER-PASS (mean_sharpe 0.82, path_t 10.6, hac_p 0.0000); CAPITAL-FAIL only structurally (live-paper + n_folds≥10). corr-to-live-ETF-trend 0.25; Track-B dSR +0.17 (0.72→0.89) — strongest diversification the program has produced.** **TREND: full-sample 0.83 but ENTIRELY pre-2010 (1977-99 +1.33, 2000-09 +1.02; 2010-19 −0.03, post-2015 +0.02) → fails the sub-period stability guard → not standalone; it's the crisis-convex partner (corr-to-carry 0.10). TREND+CARRY equal-risk book Sharpe ~1.0, post-2015 +0.57, DD −29% (vs −39%/−50% alone).** Deep-dive caught + handled: difference-adjustment (27/105 markets negative back-adj close), the CL 2020 negative-oil −306% blowup (winsor ±50%), carry PIT (signal.shift(1); expiry proxy = ex-ante schedule, not price look-ahead). 18 tests; app/ flake8 clean; report-only. **→ next: paper-deploy the futures trend+carry book to accrue the live-paper record CAPITAL needs.** See DECISIONS 2026-06-18 (P4-2) + ALPHA_V9_ROADMAP §P4-2.

> **2026-06-17 Alpha-v9 P3-5 — FINRA daily short-volume: signal is REAL but a real-but-weak near-miss (NOT a standalone edge).** Replaced the power-starved bi-monthly short-INTEREST (G2, killed for power ~190 obs) with FINRA daily off-exchange short-VOLUME (free; `cdn.finra.org`, 2019-01-02→2026-06-17 = 1,875 days, ~10× the power). Built `app/data/finra_short_volume.py` (downloads+caches the daily CNMS files → distilled daily panel: aggregate market short-vol ratio + per-symbol) + `app/research/short_volume.py` (aggregate-timing signal + frozen pre-registered verdict `P3-5-SHORTVOL-AGG`) + `scripts/run_short_volume.py`. **Scoping: AGGREGATE timing (market-wide ratio → time SPY), NOT cross-sectional** — a per-name L/S on free data is survivorship-biased (Norgate/P4-1 fixes that); the aggregate ratio is a market-wide sum, bias-free. **Result: the signal is genuinely REAL but NOT a standalone edge.** Pre-registered overlay PASSES (net Sharpe 0.99 > buy-hold 0.92, HAC p 0.001) and the informed-short prior holds (next-day SPY ret by short-vol z tercile: high −0.009% < low +0.096%; opposite direction −0.34 Sharpe; robust ex-COVID). **BUT** the deeper rigor disqualifies a standalone sleeve: residual alpha vs SPY is insignificant (+2.6%/yr, HAC t 0.95 — mostly timed beta, β 0.83) and it is **not sub-period stable** (H1 2019-22 −0.06 vs buy-hold; H2 2022-26 +0.23). **Honesty note:** my first pre-registered criterion was too lenient (mostly-long-SPY clears HAC p in a bull market) → rather than overturn the frozen PASS (goalpost-moving), I enriched the verdict with the project's own rigor (Ruler-v2 `multifactor_alpha` residual-α + the F3 sub-period guard) to set an honest `standalone_edge=False` without changing the pre-reg output. **→ route the signal to the P3-4 risk-premia composite (gate the basket) + a cross-sectional test post-Norgate.** We now own a powered short-volume data layer (G2 power problem solved). 14 tests; app/ flake8 clean; report-only, no live change. See DECISIONS 2026-06-17 (P3-5) + ALPHA_V9_ROADMAP §P3-5.

> **2026-06-16 Alpha-v9 P3-3 — overnight vs intraday decomposition: premium is REAL but cost-killed → ❌ KILL (clean).** Built `app/research/overnight_intraday.py` (decompose each name into overnight close→open + intraday open→close legs; equal-weight-universe legs; cost-grid sweep; frozen pre-registered verdict `P3-3-OVERNIGHT-INTRADAY`) + a symmetric `intraday_premium_backtest` in `app/strategy/calendar_premia.py` + `scripts/run_overnight_intraday.py`. **Frozen criterion:** on the equal-weight liquid-ETF universe (SPY/QQQ/IWM/EFA/EEM/DIA, 2007→, 4891 days), the OVERNIGHT leg NET of a realistic 1.0bps/side round-trip (2bps/day) must show net Sharpe ≥0.30 AND net CAGR>0 AND beat intraday. **Result — the overnight effect is genuinely real (overnight gross Sharpe +0.53 / CAGR +6.4% vs intraday +0.27 / +3.0% — the equity premium does accrue overnight), but the daily-round-trip cost cliff erases it: net Sharpe +0.53→+0.34 (0.5bps)→+0.16 (1bps)→−0.22 (2bps)→−1.35 (5bps).** At realistic cost overnight net Sharpe = +0.16 < 0.30 → **KILL** (turnover is the whole risk, as the roadmap predicted; even at ZERO cost +0.53 is below a compelling standalone bar and is largely overnight market beta). Sharpens the prior F1-OVN single-SPY FAIL with the full decomposition + cost cliff. **Opus deep-dive (SHIP, no bugs):** the key validity check — OHLC adjustment-basis consistency — passed empirically (overnight∘intraday reconciles to close-to-close at machine epsilon, max residual 2.2e-16 over 4891 days → no mixed-adjustment look-ahead in the split); PIT correct (overnight uses prior close, intraday same-day); cost cliff monotone. 11 P3-3 tests + 3 calendar-premia intraday tests; report-only, no live change. See DECISIONS 2026-06-16 (P3-3) + ALPHA_V9_ROADMAP §P3-3.

> **2026-06-16 Alpha-v9 P3-1 — crypto trend sleeve: falsifiable criterion MET (real low-corr stream), but PAPER-CANDIDATE not capital.** Built `app/data/alpaca_crypto_provider.py` (Alpaca spot crypto daily closes, ~2021→, partial-bar dropped for PIT) + a pre-registered `crypto_trend` Sleeve (`scripts/walkforward/sleeves.py`): TSMOM on 10 Alpaca pairs (BTC/ETH/SOL/LTC/BCH/AVAX/LINK/UNI/AAVE/DOGE), **long-flat, ann=365 + calendar lookbacks 30/90/180/365, hard book_vol_target=0.20, 25bps cost, n_trials=1**. **Standalone: Sharpe 0.64 (365-ann), annVol 18.7%, maxDD −28%, corr-to-ETF-trend 0.18 → the pre-registered criterion (Sharpe>0 AND corr<0.6) is MET = a real, lowly-correlated return stream.** Formal Sleeve Lab: **Track-A PAPER PASS** (point_SR 0.96 [365-clocked], hac_p 0.038, regime-waived); **CAPITAL FAIL** (power floor: ~5y history, n_folds<10, no live-paper — honest); **Track-B vs the live trend book FAIL** (appraisal_IR −0.27, dSR −0.17 — does NOT improve the trend-only book). **Honest verdict: PAPER-candidate — a genuine low-corr stream that doesn't yet earn capital; run live-paper to accrue the OOS record the short backtest can't give; revisit as a multi-sleeve-book diversifier.** Two Opus deep-dives (SHIP): **C1 (CRITICAL)** the gate annualized the 365-day series at 252 → threaded `periods_per_year` through Sleeve→evaluate_sleeve→ruler_v2 (point_SR 0.80→0.96; **equity sleeves unchanged at 252 — hac_p annualization-invariant, 201 gate tests pass**); **M1** drop the partial in-progress UTC bar (PIT); **M3** 2× cost (50bps) → Sharpe 0.557 (robust). 7 P3-1 tests; **full suite 3611 pass**; report-only, no live capital change. See DECISIONS 2026-06-16 (P3-1) + ALPHA_V9_ROADMAP §P3-1. **UPDATE 2026-06-16 — LIVE-PAPER ENABLED (report-only, NO capital):** built `app/live_trading/crypto_paper_track.py` — a forward-OOS recorder that recomputes the rules-based crypto book on live Alpaca closes weekly and freezes the slice from an inception date (started 2026-06-16; never back-dated into the backtest), reporting Sharpe-to-date vs the 0.64 backtest. Rules-based + PIT → the recomputed forward slice is a genuine OOS record with NO orders/capital/risk-cap interaction. `pm.crypto_paper_enabled=true` (default on), orchestrator-wired 09:55 ET (24/7, no market gate), `crypto_weekly` email, CLI `scripts/crypto_paper_report.py`. ⚠️ scheduled job needs a uvicorn restart (runnable manually meanwhile). 9 tests; cash/notifier/orchestrator/config suites 99 pass; app/ flake8 clean. See DECISIONS 2026-06-16 (P3-1 live-paper).

> **2026-06-16 Alpha-v9 P2-4 — calibrated, moneyness/DTE-aware option spread cost model (framework shipped; calibration PRELIMINARY).** Replaced the flat 1%-of-premium option half-spread with an EMPIRICAL surface (`app/options/spread_model.py::CalibratedSpreadModel`) calibrated from the live NBBO log (`data/options_spread_obs.parquet`, P1c fuse): median RELATIVE spread bucketed by (underlying × moneyness × DTE × call/put), hierarchical fallback (per-underlying → panel → marginals → CONSERVATIVE p75 global). `CalibratedOptionsSpreadCostModel` charges half the relative spread × premium (sim passes per-contract context); flat 1% stays the default (calibrated opt-in); `scripts/calibrate_option_spreads.py` regenerates it. Calibrated on 209,632 obs → **SPY ATM 30D ≈ 0.86% half-of-premium, deep-OTM ≈ 33%, global median 5.95% / conservative p75 15.8% full** — i.e. it correctly costs liquid index VRP cheaply and single-name/OTM (the OPT-3 earnings kill) dear. **Two Opus deep-dives (SHIP).** First found 3 CRITICAL, all fixed: (C1) the surface is a ~4-day 2026-06 calm-tape/indicative snapshot → ANACHRONISTIC applied to multi-year history (biases backtests CHEAPER in stress) → `calibrated_from` + `covers_date()` guard + loud docs + **NOT wired to any live VRP verdict**; (C2) global fallback was the median (too cheap for the illiquid contracts that reach it) → conservative p75; (C3) it's moneyness/DTE-aware NOT IV-bucketed (feed IV ~30% NaN) — "IV-aware" is satisfied by the premium-% base only; name/docs corrected. Second deep-dive verified all fixes + caught a latent NaN-dte guard gap (fixed). 16 P2-4 tests; **full suite 3603 pass.** Blocking prerequisite for the Phase-3 VRP engine — the framework is correct; a VRP go/no-go needs a mature surface (NBBO log spanning the test window) + live-paper validation + the per-trade covers_date guard wired in. See DECISIONS 2026-06-16 (P2-4) + OPTIONS_PROGRAM (P2-4 row).

> **2026-06-16 Alpha-v9 P1-1 — explicit cash/T-bill sleeve (default OFF).** Built `app/live_trading/cash_sleeve.py` + `cash_tracker.py`: parks idle SETTLED cash (beyond a `pm.cash_buffer_pct` liquidity buffer) into a T-bill ETF (SGOV/BIL) so the ~50% idle book earns the risk-free rate instead of zero; sells T-bills to refill the buffer when cash dips below it (risk-off path). Weekly at 09:50 ET after trend. **Architecture: T-bills are cash-equivalents → EXCLUDED from the 80% risk gross cap + open-position counts + strategy-budget buckets + sector concentration** (across risk_manager, portfolio_manager, trend_sleeve, risk_rules; keyed off `CASH_ETFS`) so parking idle cash never starves trend/PEAD/swing. Tagged selector/trade_type='cash' → Trader exit loop + startup reconciler leave them alone. **Opus deep-dive = DON'T-SHIP → fixed:** the gross-exclusion had been applied to only 2 of 6 position-counting gates (would silently zero-size the live risk book on enable), the Trader had a SECOND synthetic-adoption path that would attach a 2%/6% stop and liquidate the T-bill buffer, and the price fetch reused trend's SPY-anchored `_fetch_prices` (always None for the cash universe → risk-off sell silently no-ops); all resolved, settlement race hardened (`min(cash, buying_power)`), T+1 same-day-liquidity hole documented. 14 tests; full suite 3578 pass. Report-only until owner flips pm.cash_enabled. *(Idle cash now earns the bill rate by construction; a formal "trend + T-bills" benchmark object is a deferred follow-up.)* **Phase 1 buildable work COMPLETE** (P1-1/P1-2/P1-4 done; P1-3 shadow verdict ~mid-July). See DECISIONS 2026-06-16 (P1-1).

> **2026-06-16 Alpha-v9 P1-2 + P1-4 — trend allocation 25%→50%; live-vs-sim back-validation instrument shipped.** **P1-2:** a Kelly/vol-target analysis (`scripts/analyze_trend_allocation.py`) on the real 19.4y trend book (100%-gross Sharpe 0.72, ann vol 9.34%, maxDD −13.9%) showed **Kelly is not the binding constraint** (full Kelly = 7.7× gross / 774% of NAV; even a heavy Sharpe haircut stays >400%), so 25% (2.3% standalone vol) far under-deployed the only live edge. Owner approved **50%** (~4.7% standalone vol / ~−7% maxDD — deliberate doubling, deeply Kelly-haircut, VIX-governor-protected, under the 80% cap); flipped in the live DB + schema default. **P1-4:** built `app/live_trading/back_validation.py` — a date-matched **intended-vs-actual** tracking-error instrument. An independently-reconstructed backtest "sim" was REJECTED (first Opus deep-dive) as contaminated: it rebalances on a different calendar (deep-history modular grid, not live Mondays) and uses split/div-ADJUSTED yfinance closes vs Alpaca RAW marks (spurious ex-div returns on a bond-heavy book). Instead the sleeve's OWN intended weights (captured at rebalance: inverse-vol × alloc × governor, clipped to the per-name cap) are replayed on the SAME Alpaca price panel + SAME calendar as the actual held book, so the residual is pure execution friction (rounding, gross/per-name caps, PEAD crowding, partial/failed fills, timing). Reports correlation / annualized tracking error / drift / execution-drag + a PASS/WATCH/FAIL/BUILDING verdict; daily EOD snapshot + weekly email wired into the orchestrator; CLI `scripts/back_validation_report.py`. **Second Opus deep-dive caught a CRITICAL** — shadow runs (default mode pre-arming) carry `status='ok'` + intended_weights but place no orders, so recording their intent against an empty actual book would scream FAIL exactly while behaving as designed; fixed to record only genuine LIVE rebalances. Also fixed a latent bug: `trend_weekly` emails were silently dropped (unregistered notifier event). 17 P1-4 tests + 3 P1-2; **full suite 3564 passed / 8 skipped / 0 failures.** Both report-only; the allocation flip is the only live change (owner-gated). Trend went live ~2026-06-15 so the back-validation verdict reads BUILDING until ~15 trading days accrue (~early July). **Phase 1 remaining: P1-1 (cash sleeve) + P1-3 (credit-overlay shadow verdict ~mid-July).** See DECISIONS 2026-06-16 (P1-2, P1-4) + ALPHA_V9_ROADMAP §Phase 1.

> **2026-06-16 Alpha-v9 P0-3 — flipped `REQUIRE_TRUE_WF_FOR_PROMOTION=True`; Phase 0 (validate-the-validator) COMPLETE.** Governance close-out: a TRAINED model can no longer reach live on a *frozen-mode* generalization test (one model fit through a cutoff, scored on later folds) — it must per-fold-retrain, which is the only genuinely out-of-sample path for a fitted model. **The load-bearing fix is the true-WF predicate:** `cpcv.run_cpcv` and `engine.FoldEngine` now both set `is_true_walkforward = per_fold OR rules_based`, where rules-based = no fitted model (`is_trained=False` or `model.trained_through==date.min`) → OOS by construction. Without this, flipping the flag would have wrongly blocked every rules-based sleeve (carry/tsmom/calendar) promotion. The predicate is fail-safe: a misconfigured trained run that fails to declare per-fold stays `False` (blocked), never silently promoted. **Independent Opus 4.8 deep-dive — CRITICAL (C1):** under the live `GATE_MODE`, a frozen run's `gate_outcome()` returned `RETIRE` (a metric-failure verdict), which would make `retrain_cron._restore_previous` roll back the freshly-trained champion. Fixed in `gates.py::gate_outcome`: a run blocked SOLELY by the true-WF requirement → `GateOutcome.INCONCLUSIVE` (report-only) under ANY gate mode — "not evaluable for live promotion", never an auto-retire. **(C2)** 7 promotable-run test fixtures across 5 files updated to declare `is_true_walkforward=True` (they represent real promotions; the prior default-False masked the requirement). **(L1)** `FoldEngine` true-WF made consistent with `run_cpcv`. **Latency note:** retrain is currently DISABLED (`RETRAIN_WEEKDAY=-1`, `SWING_ENABLED=False`), so C1 was latent and this changes NO live behavior today — fixed for correctness regardless; promotion stays owner-gated. +5 P0-3 tests; **full suite 3544 passed / 8 skipped / 0 failures.** With P0-1 (pipeline-fidelity PASS) and P0-2 (gate-flaw fixes), **Phase 0 is COMPLETE — next is Phase 1 (harden & monetize: cash sleeve, trend-allocation 👤, credit-overlay 👤, live-fill back-validation).** See DECISIONS 2026-06-16 (P0-3) + PIPELINE_ARCHITECTURE changelog + ALPHA_V9_ROADMAP §Phase 0.

> **2026-06-16 Alpha-v9 P0-2 — fixed the two diversifier-killing gate flaws; carry F3 now clears as a PAPER-probation diversifier.** The external review found Ruler-v2 was quietly killing genuine diversifiers three ways. **Ⓑ The binary both-halves stability guard** (`SR(H1)>0 AND SR(H2)>0`) is unpowered — simulation confirms a **22% false-negative rate on a TRUE SR-0.5 edge** (vs the ~24% the review predicted), and it killed carry (H1 +0.69 / H2 −0.10). Replaced by `app/research/stability.py::stability_test`: a POWERED test where "stable" = the two half-Sharpes are not statistically distinguishable (stationary-bootstrap CI of SR(H1)−SR(H2) overlaps 0) → **3.8% FN** on the same true edge, while a genuine break is still caught ~4/5 draws. **Ⓒ Track-B's `standalone_vt_SR>0.20` floor** selected AGAINST diversifiers (which can have low/negative standalone SR — that's WHY they diversify); it's now report-only for declared `{diversifier, risk_premium}` (the implausibility CEILING still gates look-ahead). **Ⓓ Track-B's `P(ΔSR>0)≥0.90`** is too strict for a small-size diversifier (asymmetric loss); a `probation` path judges it vs `0.75` → a PAPER-only, small-size, **live-paper-ratified** admit (CAPITAL still structurally requires live-paper, so probation can't reach capital alone). **Success criterion MET:** re-running carry F3 (`F3-CARRY-CONFIRM-20260614`, declared risk_premium) through the new gate → **`PROMOTE_PAPER_PROBATION`**: Track-A PAPER FAIL (point_SR +0.342, hac_p 0.083 — standalone significance still misses, as expected) but Track-B PASS-probation (IR +0.434, P(ΔSR>0) 0.930, corr +0.038, standalone floor waived) AND powered-stable (H1 +0.689 / H2 −0.098, diff +0.787, CI [−0.13, +1.64] overlaps 0 — the kill-for-instability was NOT statistically justified). So carry is admitted at small size on PAPER with live-paper ratification required — exactly the roadmap's prediction that "the old guard was the problem." Honest framing: this does NOT claim carry is good — it says the binary kill was over-confident given ~9y of data; the live-paper track record (at small size, low risk) now adjudicates whether H2's weakness persists. **2 independent Opus deep-dives**: CRITICAL — `stability_test.passed` must require pooled-HAC significance so a pure-noise sleeve (trivially indistinguishable halves) can't pass as "stable" (fixed; the carry caller uses `halves_indistinguishable` + the Track-B edge instead, which is why carry — whose pooled HAC misses — still admits via book-delta); HIGH — added a `weakly_powered` transparency flag. 11 new tests + 630 gate-area tests green. Report-only; no live trading behavior changed (promotion stays owner-gated). See DECISIONS 2026-06-16 (P0-2) + PIPELINE_ARCHITECTURE changelog.

> **2026-06-16 Alpha-v9 P0-1 — positive control on the feature→label pipeline: PASS (the path is FAITHFUL; "mined out" stands on the feature side).** Built `scripts/walkforward/positive_control.py` — the keystone of Phase 0. It pushes KNOWN cross-sectional anomalies (12-1 momentum→`momentum_252d_ex1m`, short-reversal→`momentum_5d`, low-vol→`volatility`) through the **REAL** `ModelTrainer.build_train_matrix_for_window` + `engineer_features` (the feature-construction seam that `gate_calibration`'s controls — tsmom/xmom — all BYPASS by computing signals straight from prices), then measures the per-cohort rank-IC of each pipeline feature column vs the pipeline's own realized forward return (`meta.outcome_return`) and compares it to an INDEPENDENT from-raw-prices recompute (the reference IC). `use_feature_store=False` + temp-MODEL_DIR ensure `engineer_features` is actually exercised (no cache/checkpoint bypass). **Two independent Opus 4.8 adversarial deep-dives → 8 fixes** (entry-date cohort keys, production `lambdarank` label scheme so label-fidelity is a real test, mandatory-momentum in smoke, significance cohort-floor, scoped finite check, average-tie ranks, checkpoint isolation, apples-to-apples reference IC). **A critical design refinement the real run forced: the verdict is FIDELITY-centered** — a deflationary BUG is "the data has the effect but the pipeline loses it" (pipeline IC diverges from reference IC), NOT "the anomaly isn't tradeable now" (regime-dependent). **Verdict: PASS.** SMOKE (synthetic, injected effects): the detector recovers momentum (IC +0.073, t 4.31) and low-vol (−0.069, t −3.47) through the real builder, label-fidelity +0.860, and provably FAILs on a corrupted join (detection power proven). FULL (146 real R1K names, 4y, lambdarank): label-fidelity +0.761 (label seam intact); **all 3 anomalies' pipeline IC matches the independent reference within tolerance** (momentum −0.029 vs −0.031; reversal −0.015 vs −0.022; low-vol +0.018 vs +0.032) → **3/3 faithful, 0 deflationary**. The anomalies are weak/absent in 2022-2026 (regime: the 2022 momentum crash + 2023-24 mega-cap concentration), but the pipeline FAITHFULLY reproduces that reality. **Confirmed on a 12y window too** (146 names, 2014→2026): again 3/3 faithful, 0 deflationary → PASS, anomalies weak across BOTH regimes (verdict is window-robust). The reference IC is apples-to-apples (same truncated window the pipeline sees), so "faithful" = "the pipeline computes its own feature correctly"; the feature being a ~42-bar momentum is the separate P5-relevant limitation below. **Implication:** the prior "all ~23 early harness bugs inflated results → no real edge wrongly killed" claim — downgraded to UNSAFE by the external review (we had *deflationary* bugs too, e.g. #PERFOLD2 empty-X) — is now RESOLVED: IC≈0 in the swing model is the market/model/cost, **NOT** a deflationary feature-pipeline bug, so the equity-ML kills do **not** need re-opening on pipeline-bug grounds. **Genuine finding surfaced (not a bug):** `momentum_252d_ex1m` is a MISNOMER — on a ~63-bar `window_df` with a 21-bar skip it is really a ~42-bar momentum; **every swing feature lookback is clamped to the ~63-bar window, so the pipeline structurally never sees true 12-month momentum** — a candidate explanation for weak long-horizon-anomaly capture, flagged for P5. 12 tests (3 IC-primitive, 4 fidelity-classifier, 5 integration incl. corruption-FAIL + offline-fetch). See `ALPHA_V9_ROADMAP.md` §2 Ⓐ.

> **2026-06-15 Alpha-v8 G3 — additive long-flat credit-timing SLEEVE: PARK (real standalone edge, but the corr<0.30 wall).** `app/strategy/credit_curve_governor.credit_timing_returns` (hold SPY when HY ≥ trailing MA, flat when credit-stressed) run through the FULL Sleeve Lab gate vs the live trend book, 2007→2026 (n=3612 OOS). Verdict (`G3-CREDIT-TIMING`): **Track-A PAPER PASS** (point_SR **+0.913**, HAC p(1s) 0.0001, mean_SR +1.03) — a genuine standalone edge — BUT **Track-B FAIL** on the **correlation wall (corr_to_book +0.518 ≥ 0.30)** + P(ΔSR>0) 0.873 (<0.90). The high SR is largely SPY BETA (residual-α_t +1.50 < 2.0), so it can't diversify a trend book that already holds SPY. Independent Opus adversarial review: **PARK honest** — no look-ahead (shift(1) PIT, trailing MA), the 0.91 SR is the real consequence of "long SPY + step aside in credit stress" (beta-plus-timing), corr 0.52 is the decisive structural fail; the `diversifier` label is generous (it's directional beta, worst_regime −1.978) but it FAILS Track-B under either label. **Decision: PARK** — not promoted (fails Track-B), not killed (real standalone merit). **The credit signal's productive form is the G1 OVERLAY, not an additive sleeve** — confirming, once more, the corr/IC wall that additive equity sleeves keep hitting. See DECISIONS 2026-06-15 (G3).

> **2026-06-15 Alpha-v8 G2 — aggregate short-interest de-risk overlay (RRT): KILLED (honest true-negative).** Built `app/strategy/short_interest_governor.py` (market-level aggregate short interest → PIT-safe trailing-z Short Interest Index → de-risk when shorts crowded) on the existing `short_interest_provider` cache (Polygon/FINRA, bi-monthly, PIT `knowable_date`). **Depth gate (G2a): GO** (202 settlement dates, 2017-12-29→2026, survivorship-safe) — but POWER-CAVEATED (bi-monthly, NO GFC, only ~3 in-window crises). Verdict (`G2-SHORTINT-OVERLAY`, marginal to the VIX governor): **KILL.** De-risks 49% of days at z>1; **uniformly Sharpe/Calmar-NEGATIVE across a 12-cell grid** (window{24,48}×z{1,1.5,2}×derisk_to{0.5,0.75} — every cell improves_tail=False, dSharpe −0.012…−0.141, dCalmar −0.04…−0.38); **COVID dd_improve +0.000** (the bi-monthly signal + 10bd lag structurally can't react to a fast crash). Independent Opus adversarial review: **KILL honest** — sign correct (RRT-oriented), PIT clean (0 lookahead mismatches), no aggregation bug; the decisive finding is that in the 2018+ window **high-SII days had HIGHER forward returns** (the RRT effect is absent/reversed post-publication), so de-risking the better half is uniformly Sharpe-negative. No config rescues it (unlike credit). **Line closed.** Footnote: only the market-TIMING overlay is killed; the per-name SI data layer (`latest_known_si`/`get_short_interest_at`) remains sound + reusable for a future cross-sectional SI factor. See DECISIONS 2026-06-15 (G2).

> **2026-06-14 Alpha-v8 G1 — credit/curve de-risk overlays (marginal to the live VIX governor).** Built `app/strategy/credit_curve_governor.py` + the G0 marginal-stacking API. Evaluated each overlay MARGINAL to the governor (book×gov×candidate vs book×gov) on the live trend book, 2007→2026. **CURVE — KILLED** (`G1-CURVE-OVERLAY`): 10y−3m inversion is inert/negative (marginal dMaxDD −0.000, dSharpe −0.017, both-halves fail) — too slow to be tactical. **CREDIT, pre-registered config (L60/band0) — KILLED** (`G1-CREDIT-OVERLAY`): fires 37% of days (a slow trend filter, not a stress signal), shallows DD but Calmar/Sharpe-negative marginal to the governor (dCalmar −0.071). **CREDIT, selective re-spec (L120 / 2%-below-MA, ~18% of days) — ⭐ promote_paper CANDIDATE** (`G1B-CREDIT-SELECTIVE`, parent=the killed L60): marginal-to-governor **dSharpe +0.064, dCalmar +0.030**, all 3 crises improve (GFC +0.019, COVID +0.022, BEAR +0.006), and the **pre-registered both-halves stability guard PASSES robustly (H1 +0.069, H2 +0.047 — validating on DIFFERENT crises per half)**. Independent Opus adversarial review: **promote_paper HONEST + DEFENSIBLE** — PIT-clean, genuinely non-redundant to the VIX governor (de-risks on a slower credit axis), multiplicity (1 of 2 survivors from a 12-cell grid) mitigated by the both-halves split + 3-crisis breadth + economic prior. **6 recorded caveats:** post-hoc/multiplicity (pre-reg L60 failed); small effect (+0.064 Sharpe, ~0 maxDD — a tail/insurance overlay, not a return engine); lookback fit-sensitive (L120 works, L60 fails; band robust); both-halves dSharpe is supportive evidence not a gated criterion; **owner-gated PAPER only — never capital** (overlays bypass Track-A significance); live wiring must use `live_credit_multiplier` (fail-safe). **First Alpha-v8 candidate → owner-gated G4 wiring (flag default OFF; NOT flipped live).** See DECISIONS 2026-06-14 (G1).

> **2026-06-14 Alpha-v7 F3-CARRY-CONFIRM — owner-authorized fresh confirmation → KILLED (the edge is a pre-2016 artifact).** Pre-registered (R7) `F3-CARRY-CONFIRM-20260614` (confirmatory, family carry; registry decision=kill, run_at strictly after preregistered_at). The principled spec: **long-flat IEF / scale 1.5 / 1bp** — chosen on ECONOMICS (harvest positive carry, stand aside on inversion; deliberately NOT the highest grid cell TLT/2.0/long-short), criteria frozen BEFORE the run. Result: full-sample point_SR +0.342, residual-α +2.27, **Track-B PASS** (IR +0.434, P(ΔSR>0) 0.930, corr +0.038 — long-flat clears the bar the long-short canonical missed). BUT Track-A PAPER still FAIL (HAC p 0.0833 > 0.05) AND — decisively — the **pre-registered sub-period stability guard FAILED: H1 2007-2016 SR +0.689 vs H2 2017-2026 SR −0.098.** The carry edge is ENTIRELY a first-decade artifact, gone/negative in the modern (post-2016, rate-hike-cycle) regime. Pre-registered decision rule → **KILL.** The stability check did exactly its job — caught a config-robust but TIME-unstable edge that the full-sample near-miss masked; without it (or by cherry-picking the grid) a dead-since-2016 sleeve could have been wrongly promoted. **Owner-decision #2 resolved: carry line CLOSED.** Net of the whole F-series, the VIX governor (overlay) is the ONLY surviving candidate. Runner: `scripts/run_carry_confirmation.py`. See DECISIONS 2026-06-14 (F3-CARRY-CONFIRM).

> **2026-06-14 Alpha-v7 F-series WRAP — F4 deferred + F5 not-triggered; the sweep empirically confirms "trend is the only standalone edge".** **F4 (options-conditioned event interaction) DEFERRED:** the thesis was already tested and KILLED in P4 — H4a–H4e (all 5 options-conditioned event hypotheses) ALL KILL + PEAD demoted at event level (t=−0.77) — on the SAME frozen 4y options store (2022–2026), which is underpowered by construction (can't clear the power floor or deep-history significance). The panel itself ranked it a long-shot ("conditioning only"). Re-running a continuous-interaction variant on already-killed, underpowered data is negative-EV; deferred (revisit only with longer options history or a new event signal). **F5 (book assembly) NOT TRIGGERED:** gated on "≥2 sleeves pass the pre-registered gate"; ZERO did (carry near-missed; calendar premia + relative-value failed; the governor is an overlay, not a sleeve). **Illustrative owner-pending book (2007→2026, NOT promoted):** trend alone SR 0.723 / maxDD −13.9% / Calmar 0.469; trend+carry (naive vol-wt) SR 0.689 / maxDD −11.3% (corr trend~carry +0.029 — genuinely uncorrelated, but naive vol-weighting de-levers so it understates); +governor overlay maxDD −10.9%. carry's low standalone SR limits the book-SR lift (modest — consistent with "book SR 0.7–0.9, not a home run"); a proper equal-risk re-levered book is the real F5 build, which triggers only on a passing sleeve. **F-series scoreboard:** F0 Lab ✅ (permanent infra) · F1a calendar premia ❌ · F1b VIX governor 🟡 helps (overlay; owner-gated) · F1c FOMC drift ⏸ deferred · F2 ETF rel-val ❌ · F3 carry 🟠 robust near-miss (owner-rec re-registration) · F4 options ⏸ deferred · F5 ⏸ not-triggered. **Bottom line: no additive sleeve cleared the pre-registered bar on free daily US data — confirming the panel's thesis empirically. Two real-signal items remain (governor + carry), both owner decisions. If both stall, the honest next lever is a deliberate DATA buy (Norgate futures / longer options), not more searching.** See DECISIONS 2026-06-14 (F4 + F5 wrap).

> **2026-06-14 Alpha-v7 F3 — rates duration carry: NEAR-MISS (real, robust, orthogonal edge — but the pre-registered config fails both bars → NOT promoted).** `app/strategy/carry.py` — size a duration-ETF (IEF) position by the term spread (10y−3m from yfinance ^TNX/^IRX), continuous long-short, PIT-lagged; declared `risk_premium`. Canonical config (IEF, scale_pct=1.5, long-short, 1bp), deep history 2007→2026 (~3661 pooled-OOS): point_SR **+0.314** (clears the 0.30 floor), mean_SR +0.221, path_t +1.87, HAC p(1s) **0.0998**, **residual-α_t +2.10** (FIRST sleeve with genuine positive alpha vs SPY; beta −0.09). **Track-A PAPER FAIL on hac_significance only** (p 0.0998 > 0.05). **Track-B vs trend book: 7/8 PASS — appraisal_IR +0.386, corr +0.010 (genuinely orthogonal), ΔSR +0.084 (additive) — fails ONLY P(ΔSR>0) 0.886 (< 0.90), by 0.014.** Misses BOTH pre-registered bars marginally → FAIL, NOT promoted. **Robustness grid (12 configs, etf∈{IEF,TLT} × scale∈{1.0,1.5,2.0} × long-short∈{T,F}): the edge is REAL and ROBUST — positive orthogonal residual-α in ALL 12 (resid-t +1.77…+2.70, corr +0.006…+0.045), and Track-B PASSES in 8/12 neighbors; the canonical IEF/1.5/long-short is one of the 4 that narrowly fails.** Independent Opus adversarial review: HONEST — no look-ahead (shift(1) PIT confirmed), NO unit bug (^TNX/^IRX both in %, real term spread median +1.56%, 51% interior positions = genuine curve timer not static beta), residual-α real (beta −0.09), cost-insensitive (SR 0.41→0.38 from 1→5bp). **DISCIPLINE: the pre-registered config is the verdict (FAIL); I did NOT switch to a passing config. Owner-recommended next step = a FRESH pre-registered carry confirmation (new hypothesis_id + R7 cooling-off), NOT a cherry-pick from this grid.** Strongest ADDITIVE candidate of the F-series. See DECISIONS 2026-06-14 (F3).

> **2026-06-14 Alpha-v7 F2 — ETF relative-value (slow spread mean-reversion): FAIL (genuinely orthogonal, but no standalone edge).** `app/strategy/etf_relative_value.py` — dollar-neutral log-spread MR over 5 pre-registered economically-linked pairs (QQQ/SPY, IWM/SPY, HYG/IEF, TLT/IEF, EEM/EFA), canonical L=120 / entry z=1.5 / exit 0.5 / 2bp-per-leg, equal-weight, declared `diversifier`. Deep history 2007→2026 (~3661 OOS): point_SR +0.026, mean_SR +0.110, path_t +0.62, HAC p(1s) 0.458. **PAPER FAIL (point_sr_floor + significance); Track-B FAIL** — corr **−0.230** (genuinely orthogonal — the diversification the F1 calendar premia LACKED) but standalone SR ≈0 → appraisal_IR +0.10, ΔSR −0.011, P(ΔSR>0) 0.46 (a zero-return diversifier adds nothing). Grid diagnostic: EVERY {L∈60/120/250 × entry∈1.0/1.5/2.0} cell stays below the 0.30 floor net of cost (best L=60 net SR 0.19); per-pair only IWM/SPY +0.22 & EEM/EFA +0.20 positive → the combine isn't hiding a winner. Independent Opus adversarial review: HONEST, no bug (spread SIGN correct, PIT clean, cost fair 4bp/round-trip). **No promotion.** See DECISIONS 2026-06-14 (F2).

> **2026-06-14 Alpha-v7 F1b — VIX-term crash governor: MODESTLY HELPS (first positive; candidate overlay, owner-gated).** Built the OVERLAY eval path in the Sleeve Lab (`Overlay` / `evaluate_overlay` / `OverlayReport` — book-WITH vs WITHOUT, tail metrics, NOT Track-A significance — the dedicated path promised in F0) + `app/strategy/crash_governor.py` (de-risk the book when VIX>VIX3M = backwardation; PIT signal at close t → applied t+1). Canonical config (derisk_to=0.5, ratio_threshold=1.0, confirm_days=1) on the live 10-ETF TSMOM book, 2007→2026 (n=4891): **maxDD −13.9%→−12.1% (+1.8pp), Calmar 0.469→0.501 (+0.032), Sharpe 0.723→0.720 (−0.003), at a ~0.5%/yr return give-up; COVID-2020 drawdown notably shallower −10.7%→−6.5%** (de-risks 11% of days, mean mult 0.944). Independent Opus adversarial review: HONEST + PIT-verified (un-shifting the signal jumps Sharpe to 1.038 — the lag removes exactly the future info), no GFC data gap (^VIX3M back-stitches VXV to 2007; 146 real GFC obs), config canonical not tuned, robust across the threshold≥1.0 neighborhood (36-cell grid — maxDD improves in nearly every cell; only threshold 0.95 = de-risking in contango fails). Caveats recorded: the real cost is the −0.5%/yr give-up (not Sharpe); the COVID tail benefit is event-concentrated. **First candidate worth the owner's consideration; promotion to live = owner decision (it re-times live exposure). NOT promoted.** See DECISIONS 2026-06-14 (F1b).

> **2026-06-14 Alpha-v7 F1a — calendar/overnight premia: BOTH FAIL (report-only, via the Sleeve Lab).** First sleeves run through the F0 Sleeve Lab (`scripts/walkforward/sleeves.py` registry + `app/strategy/calendar_premia.py`), deep history 2007→2026 (~3661 pooled-OOS obs), declared `risk_premium`. **turn_of_month_SPY** (last 1 + first 3 trading days, long-flat): point_SR +0.320, mean_SR +0.280, path_t +3.07, HAC p(1s) 0.097. **overnight_SPY** (long close→open daily, 2bp round-trip/day): point_SR +0.344, mean_SR +0.393, path_t +2.58, HAC p 0.087, residual-α_t −1.23 (largely diluted SPY beta). **Both clear the plausibility floor (point_SR ≥0.30) but MISS the light HAC significance bar (p 0.087/0.097 > 0.05) → PAPER FAIL; and decisively FAIL Track-B vs the live trend book** (overnight IR +0.09 / corr 0.33 / P(ΔSR>0) 0.47; TOM IR −0.03 / ΔSR −0.058 / P 0.25) — neither diversifies the book (overnight is timed SPY beta the book already holds). Independent Opus adversarial review: verdict HONEST, no bug — PIT clean (TOM calendar position needs no shift; overnight = open[t]/close[t-1]−1), costs fair (TOM ~2bp/yr, overnight ~5%/yr but physically real), Track-B fail cost-independent + decisive. **No promotion; recorded so we never blind-retest these.** CAPITAL unreachable by design (n_folds=8<10 + live-paper). See DECISIONS 2026-06-14 (F1a).
...
## Futures breadth sweep (FB0) — how much breadth restores the edge? — 2026-07-07 — VERDICT: ~48 MARKETS (3×) NEEDED, COMMODITY-FIRST; MARGINAL + COST-INFLATED → RECOMMEND SHELVE

`scripts/run_futures_breadth_sweep.py` (SSOT `docs/reference/FUTURES_BREADTH_PROGRAM_2026-07-07.md`). Reused the gate machinery on the full-76 panels; momentum recomputed per subset (anchor reconciles EXACTLY to the gate's −0.20; full-76 = +2.61 → method sound, Opus-reviewed). **Breadth curve (random subsets, E[t]):** k=16 +0.30 → 40 +1.42 → **48 +2.04** → 64 +2.27 → 76 +2.61 (σ~0.6) → **~48 markets (48±8) to clear t>2**. **Buckets (16 + one family, Δt):** commodities diversify most (grains +1.44, energy +1.25, softs +0.92, livestock +0.64); financials/FX hurt-or-nothing (fx −1.12, intl_rates −0.54) — the 16 is already financial-heavy, the missing breadth is COMMODITY. Greedy forward-selection was OVERFIT (picked the harmful families; removed). **Caveats (Opus): survivorship inflates all t (incl. 2.61, ~1 SE above the bar); flat 3bps understates real commodity roll/execution costs → deliverable t thinner than shown.** **RECOMMENDATION: SHELVE/DEFER futures-live** — a ~3× expansion into the hardest-to-trade commodity complex (foreign contracts, permissions, per-market roll/FND) for a marginal, cost-inflated diversifier is not worth it. Owner call: FB-SHELVE (recommended) vs FB1 (expand). The R1.3 execution stack stays valid + inert.
