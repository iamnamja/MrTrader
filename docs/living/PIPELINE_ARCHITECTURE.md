...
scripts/
  ...
  walkforward/
    ...
    sleeves.py                  — the SLEEVE REGISTRY (F1+): @register_sleeve builders (turn_of_month, overnight, etf_relative_value, …) + the VIX-term governor overlay (build_vix_term_governor/run_governor) + CLI (python -m scripts.walkforward.sleeves [name|governor]). Backtests live in app/strategy/ (calendar_premia.py, crash_governor.py, etf_relative_value.py).
    strategies/
