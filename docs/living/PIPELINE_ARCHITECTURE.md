...
- **`"ruler_v2"` (LIVE DEFAULT, owner-approved 2026-06-13):** inference ON THE OOS RETURN
  ...
  `ruler_v2_r4_summary` in `scripts/walkforward/gate_calibration.py`. **`TRACKB_MODE` stays
  `"book_delta"`** — the Ruler-v2 Track-B gate (`appraise_track_b`) is built/tested but its
  runner dispatcher is not yet wired (Phase-3 completion).
