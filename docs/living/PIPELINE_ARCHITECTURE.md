...
scripts/
  ...
  walkforward/
    ...
    track_b_appraisal.py        — appraise_track_b(): Track-B book-delta appraisal (IR + block-bootstrap P(ΔSR>0))
    sleeve_lab.py               — Sleeve Lab (Alpha-v7 F0/F1b): uniform Sleeve→run_cpcv→ruler_v2 Track-A(PAPER+CAPITAL)→[Track-B]→SleeveReport + registry; assemble_book() (F5); + OVERLAY path (Overlay/evaluate_overlay/OverlayReport — book-WITH-vs-WITHOUT tail eval for governors). Report-only.
    sleeves.py                  — the SLEEVE REGISTRY (F1+): @register_sleeve builders (turn_of_month, overnight, etf_relative_value, rates_carry, …) + the VIX-term governor overlay (build_vix_term_governor/run_governor) + fetch_yield (^TNX/^IRX) + CLI (python -m scripts.walkforward.sleeves [name|governor]). Backtests live in app/strategy/ (calendar_premia.py, crash_governor.py, etf_relative_value.py, carry.py).
    strategies/
...
### 7.0 — `GATE_MODE`: **`"ruler_v2"` is the LIVE default (2026-06-13)**

`app/ml/retrain_config.py::GATE_MODE` selects the promotion gate. **As of 2026-06-13 the
production default is `"ruler_v2"`** (Alpha-v7 Phase B), flipped from `"significance"` after
the R4 calibration came back CLEAN on the full control set. The `"significance"` and
`"mean_sharpe"` branches are RETAINED as legacy (byte-for-byte; reachable via
`CPCVResult.significance_gate_*` / explicit mode), exactly as `mean_sharpe` was kept when
significance went live.

- **`"ruler_v2"` (LIVE DEFAULT, owner-approved 2026-06-13):** inference ON THE OOS RETURN
  SERIES (not the path-Sharpe distribution). PAPER = plausibility (point-SR floor +
  implausibility ceiling + non-catastrophic regime, **WAIVED for declared diversifiers/
  risk-premia**) **+ a light one-sided HAC-SR significance floor** (`RULERV2_PAPER_MAX_HAC_P
  =0.05` on the pooled OOS series — closes the plausibility-only Type-I leak R4 found).
  CAPITAL = Bayesian posterior P(SR>0)≥0.95 (trial-shrunk prior; replaces the saturated
  DSR) **+ a STRUCTURAL live-paper requirement** + multi-factor residual-α t≥2.0 + stationary-
  bootstrap P(SR>0)≥0.95 + (PBO≤0.50 if M>1) + a hard power floor (n_obs≥504, n_folds≥10).
  PF/Calmar demoted to report-only. Full spec: `docs/reference/RULER_V2_DESIGN.md`. Modules:
  `app/research/{inference,bayes_sr,ruler_v2}.py`; gate-calibration R4 instrument +
  `ruler_v2_r4_summary` in `scripts/walkforward/gate_calibration.py`. **`TRACKB_MODE` is also
  LIVE `"ruler_v2"` (2026-06-13)** — `scripts/run_book_gate.py` now dispatches on it to
  `track_b_appraisal.appraise_track_b` (budget-invariant appraisal IR + block-bootstrap
  P(ΔSR>0); legacy `book_delta_gate` retained). run_book_gate is a manual, report-only tool
  (`decision='park'`, owner-gated).
- **`"significance"` (LEGACY since 2026-06-13; was the prior default):** the significance-first
...
| Risk budget | `TRACKB_MAX_RISK_BUDGET` | candidate blended at ≤ **25%** of book risk (amended 2026-06-11 from 10% — see changelog #451) |
...
CAPITAL (owner sign-off + a codified tail-loss budget required). **Calibration — RESOLVED 2026-06-11
(#451):** the first real run (#450) confirmed the 10% budget structurally rejects any realistic
diversifier on ΔSharpe (ΔSR ≈ budget·(SR_cand − corr·SR_book) ⇒ needs SR_cand ≳ 1.11 at b=10%/corr=
0.27). An owner-approved REGISTERED amendment raised `TRACKB_MAX_RISK_BUDGET` 10% → **25%** (the
conservative end of the 25–40% range; the Phase-2/3 book ran trend at ~40–50%). Re-run at 25%:
**TSMOM PASSES all 8 criteria** (Sharpe 0.41→0.64, Calmar 0.28→0.59, maxDD shallower); the
...
**Promotion gate.** `REQUIRE_TRUE_WF_FOR_PROMOTION` (**True since Alpha-v9 P0-3, 2026-06-16**) makes `gate_passed()` return False for any `is_true_walkforward=False` run regardless of metrics, in BOTH `WalkForwardReport` and `CPCVResult`. A run is "true WF" when it either (a) per-fold-retrains a TRAINED model, or (b) is RULES-BASED — no fitted model (`is_trained=False` or `model.trained_through==date.min`) → out-of-sample by construction; `cpcv.run_cpcv` and `engine.FoldEngine` both set `is_true_walkforward = per_fold OR rules_based`. **Consequence:** a TRAINED model can no longer reach live on a frozen-mode generalization test — it must per-fold-retrain; rules-based sleeves (carry/tsmom/calendar) are unaffected. **Report-only safety (C1):** a frozen run blocked SOLELY by this requirement returns `GateOutcome.INCONCLUSIVE` (not RETIRE) under ANY gate mode, so `retrain_cron._restore_previous` does NOT roll back the champion — "not evaluable for live promotion", never an auto-retire.
...
| `pm.crash_governor_enabled` (agent_config) | "true" | **LIVE** VIX-term crash governor (Alpha-v7 F1b). true (owner-approved) = scale the trend sleeve's budget to `derisk_to` when VIX>VIX3M (backwardation) on the last `confirm_days` SETTLED closes. false = overlay off. FAIL-SAFE: missing/stale/error → multiplier 1.0; can only REDUCE exposure (caps still bind). Needs orchestrator restart to load code | Alpha-v7 F1b |
| `pm.crash_governor_derisk_to` / `_ratio_threshold` / `_confirm_days` (agent_config) | 0.5 / 1.0 / 1 | Governor params (VixTermGovernorConfig): exposure multiplier in backwardation / VIX/VIX3M de-risk threshold / consecutive inverted settled closes required. The validated config | Alpha-v7 F1b |
| `pm.credit_governor_enabled` (agent_config) | **"false"** | Alpha-v8 G1 CREDIT de-risk overlay (HYG/IEF below trailing MA). DEFAULT OFF (owner-gated candidate). true = de-risk; composes ×VIX governor, product clamped to 0.25 floor. FAIL-SAFE → 1.0; can only REDUCE exposure | Alpha-v8 G1/G4 |
| `pm.credit_governor_derisk_to` / `_lookback` / `_band` (agent_config) | 0.5 / 120 / 0.02 | Credit overlay params (CreditGovernorConfig): the validated selective config (fire on >2% below a 120d MA) | Alpha-v8 G1/G4 |
...
| `REQUIRE_TRUE_WF_FOR_PROMOTION` | **True** (Alpha-v9 P0-3) | `gate_passed()` returns False for any non-true-WF (`is_true_walkforward=False`) run, even if metrics pass. True-WF = per-fold-retrained TRAINED model OR rules-based sleeve (OOS by construction). Frozen runs blocked by this alone → INCONCLUSIVE (report-only), never RETIRE. See KL-10b. | Live |
...
|---|---|---|---|
| 2026-06-25 | (Audit Wave 6a) | **`AgentSimulator` SIGNAL-mode shorts now reserve margin (research-tier; non-live).** A signal-mode short previously did a net-zero cash hack (`cash += proceeds; cash -= notional`) and left `short_collateral` at 0, while `buying_power` returned raw `cash` — so the RM buying-power gate saw ~full buying power after a short and the short book could over-leverage (the `buying_power` cap was defeated for shorts). Now signal-mode short ENTRY reserves `short_collateral += entry_notional` (parity with REBALANCE mode), `buying_power` returns `cash - short_collateral`, and the signal-mode cover (`_close_position`) releases the reserve and pays the exit notional. **`equity_mtm`, `equity_decision`, `_effective_cash`, and round-trip realized P&L are provably UNCHANGED** (long-only books reserve nothing → identical); ONLY the short-side buying-power gate is corrected. **Eval impact:** signal-mode LONG/SHORT backtests that previously over-levered shorts will change (lower leverage/Sharpe) → affected L/S evals should be RE-RUN; long-only swing WF/CPCV is unaffected. Opus deep-dive verified the 3 coupled edits keep cash/equity invariant. 4 tests; 375 simulator-area tests green. | `app/backtesting/agent_simulator.py`, `tests/test_wave6a_simulator_short_margin.py` |
| 2026-06-16 | (Alpha-v9 P0-3) | **Flipped `REQUIRE_TRUE_WF_FOR_PROMOTION` False→True — governance: a TRAINED model can no longer promote on a frozen-mode generalization test (it must per-fold-retrain).** Made the true-WF predicate correct + fail-safe: `cpcv.run_cpcv` and `engine.FoldEngine` now set `is_true_walkforward = per_fold OR rules_based`, where rules-based = no fitted model (`is_trained=False` or `model.trained_through==date.min`) → OOS by construction. This is the load-bearing fix: WITHOUT it, flipping the flag would wrongly block every rules-based sleeve (carry/tsmom/calendar) promotion. **Opus deep-dive CRITICAL C1:** under live `GATE_MODE`, a frozen run's `gate_outcome()` returned RETIRE (not INCONCLUSIVE) → `retrain_cron._restore_previous` would roll back the freshly-trained champion. FIXED in `gates.py::gate_outcome`: a run blocked SOLELY by the true-WF requirement → `GateOutcome.INCONCLUSIVE` (report-only) under ANY gate mode. C2: updated 7 promotable-run test fixtures across 5 files to declare `is_true_walkforward=True` (they represent real promotions). Retrain is currently DISABLED (`RETRAIN_WEEKDAY=-1`, `SWING_ENABLED=False`) so C1 was latent, but fixed for correctness regardless. **Phase 0 (validate-the-validator) COMPLETE.** Full suite 3544 passed / 8 skipped / 0 failures; +5 P0-3 tests. | `app/ml/retrain_config.py`, `scripts/walkforward/cpcv.py`, `scripts/walkforward/engine.py`, `scripts/walkforward/gates.py`, `tests/test_p0_3_require_true_wf.py`, +5 fixture files |
| 2026-06-16 | (Alpha-v9 P0-2) | **Fixed the two diversifier-killing gate flaws (Ⓑ Ⓒ Ⓓ).** (Ⓑ) New `app/research/stability.py::stability_test` — a POWERED sub-period stability test (bootstrap CI of the half-Sharpe DIFFERENCE overlaps 0 = no detectable break) replacing the binary "positive in both halves" guard (**22% false-negative on a true SR-0.5 edge** in sim vs 3.8% powered; it had killed carry). Two outputs: `halves_indistinguishable` (pure no-break, used WITH a separate edge test) and `passed` (= no-break AND pooled-HAC significant — safe standalone verdict that rejects pure noise); + `weakly_powered` flag. (Ⓒ) `track_b_appraisal`: the `standalone_vt_SR>0.20` FLOOR is now REPORT-ONLY for declared `{diversifier, risk_premium}` (a genuine diversifier can have low/neg standalone SR; appraisal-IR already certifies book-improvement) — the implausibility CEILING still gates. (Ⓓ) New `probation` path: for declared diversifiers P(ΔSR>0) is judged vs `RULERV2_TRACKB_PROBATION_MIN_PDSR=0.75` (not 0.90) → a PAPER-only, small-size, **live-paper-ratified** admit (`requires_live_paper_ratification`; CAPITAL still structurally needs live-paper, so probation can't reach capital alone). **Validation: carry F3, which the binary guard KILLED, now reads `PROMOTE_PAPER_PROBATION`** (Track-A FAIL hac_p 0.083; Track-B PASS-probation IR +0.434 / P(ΔSR>0) 0.930 / corr +0.038; stability H1 +0.689 / H2 −0.098, diff CI [−0.13,+1.64] overlaps 0). 2 Opus deep-dives (CRITICAL `passed` must require pooled-significance; HIGH `weakly_powered` — both fixed). 11 tests; 630 gate-area tests green. Report-only; no live behavior changed (promotion owner-gated). | `app/research/stability.py`, `scripts/walkforward/track_b_appraisal.py`, `scripts/walkforward/sleeve_lab.py`, `scripts/run_carry_confirmation.py`, `app/ml/retrain_config.py`, `tests/test_p0_2_gate_fixes.py` |
| 2026-06-16 | (Alpha-v9 P0-1) | **Positive control on the feature→label pipeline — VERDICT: PASS (path is FAITHFUL; no deflationary bug).** New `scripts/walkforward/positive_control.py` pushes KNOWN cross-sectional anomalies (12-1 momentum, short-reversal, low-vol) through the REAL `build_train_matrix_for_window`+`engineer_features` (the seam `gate_calibration` controls bypass) and measures per-cohort rank-IC of each pipeline feature column vs the pipeline's own `outcome_return`, compared to an independent from-raw-prices reference IC. FIDELITY-centered verdict (a bug = data has the effect but the pipeline loses it; NOT effect-absence, which is regime-dependent). `use_feature_store=False` + temp-MODEL_DIR force live `engineer_features` (no cache/checkpoint bypass). 2 Opus deep-dives → 8 fixes. FULL (146 R1K, 4y, lambdarank): 3/3 anomalies faithful (pipeline IC ~= reference IC), 0 deflationary, label-fidelity +0.761 → PASS. **Resolves roadmap Ⓐ: IC≈0 in the swing model is market/model/cost, not a feature-pipeline bug; equity-ML kills need not re-open on pipeline-bug grounds.** Surfaced finding: swing features are window-limited to ~63 bars (`momentum_252d_ex1m` is really ~42-bar) — pipeline never sees true 12-month momentum (flagged for P5). Report-only; no live/gate behavior changed. 12 tests. | `scripts/walkforward/positive_control.py`, `tests/test_positive_control.py` |
| 2026-06-15 | (G4) | **Credit overlay wired into the live trend sleeve — flag DEFAULT OFF (owner-gated).** `_credit_governor_multiplier` (HYG/IEF from macro_history, validated L120/2%/0.5; fail-safe, PIT) in `trend_sleeve.py`; overlays now COMPOSE: `overlay_mult = max(0.25, vix_gov × credit)`, `alloc *= overlay_mult` (audit `size_mult`=overlay_mult). 4 `pm.credit_governor_*` flags (`enabled` DEFAULT "false" → zero live change). Opus-verified SAFE (flag-off byte-identical to pre-G4; can only reduce exposure; no governor regression). Completes Alpha-v8. 8 tests. | `app/live_trading/trend_sleeve.py`, `app/database/agent_config.py`, `tests/test_crash_governor.py`, `tests/test_trend_sleeve.py` |
| 2026-06-15 | (G3) | **Additive credit-timing sleeve — PARK (real standalone edge, corr wall).** `credit_curve_governor.credit_timing_returns` (long-flat SPY timed by the credit signal) + `build_credit_timing_sleeve` in `sleeves.py`, run through the full Track-A+B gate vs the trend book. Verdict: Track-A PAPER PASS (point_SR +0.913, HAC p 0.0001) but Track-B FAIL (corr_to_book +0.518 ≥ 0.30 — largely SPY beta). Opus-verified honest (no look-ahead). Not promoted; credit's productive form is the G1 overlay. Completes Alpha-v8 G0→G3 research. 3 tests. | `app/strategy/credit_curve_governor.py`, `scripts/walkforward/sleeves.py`, `tests/test_credit_curve_governor.py` |
| 2026-06-15 | (G2) | **Aggregate short-interest de-risk overlay (RRT) — KILLED (honest true-negative).** `app/strategy/short_interest_governor.py` (aggregate SI → PIT-safe trailing-z SII → de-risk; + `live_si_multiplier` fail-safe) + `build_short_interest_governor`/`run_short_interest` in `sleeves.py`, on the existing `short_interest_provider` cache. Verdict (report-only, marginal to the VIX governor): KILL — de-risks 49% of days, uniformly Sharpe/Calmar-negative across a 12-cell grid, COVID un-catchable (bi-monthly + 10bd lag). Opus-verified honest (sign/PIT/aggregation correct; RRT effect absent/reversed post-2017). Only the market-timing overlay is killed; the per-name SI data layer stays reusable. 6 tests. | `app/strategy/short_interest_governor.py`, `scripts/walkforward/sleeves.py`, `tests/test_short_interest_governor.py` |
| 2026-06-14 | (G1) | **Credit/curve de-risk overlays (marginal to the VIX governor).** `app/strategy/credit_curve_governor.py` (`credit_multiplier`: HYG/IEF below trailing MA; `curve_multiplier`: 10y−3m inversion; + `live_*` fail-safe helpers) + `build_credit_governor`/`build_curve_governor`/`run_credit_curve` in `sleeves.py`. Verdicts (report-only): curve KILLED (inert); credit-orig L60 KILLED (over-eager, Calmar−); **credit-SELECTIVE (canonical `CreditGovernorConfig` = L120/band 0.02/derisk_to 0.5) = promote_paper candidate** — marginal dSharpe +0.064, all-3-crises, both-halves stable; Opus-reviewed defensible (multiplicity-disclosed; owner-gated paper only, never capital). NOT live; → owner-gated G4. 7 tests. | `app/strategy/credit_curve_governor.py`, `scripts/walkforward/sleeves.py`, `tests/test_credit_curve_governor.py` |
| 2026-06-14 | (G0) | **Alpha-v8 overlay marginal-stacking infra.** `sleeve_lab.py`: `compose_overlays` (multiply date-aligned overlay multipliers, clamp product to [GLOBAL_DERISK_FLOOR=0.25, 1.0]) + `evaluate_overlay_marginal` (judge a candidate overlay vs the already-overlaid book — e.g. credit vs book×governor; reduces exactly to `evaluate_overlay` with no prior) + an overlay registry (`@register_overlay`/`build_overlay`/`list_overlays`; the VIX governor registered as `"vix_term_governor"`). `evaluate_overlay` refactored onto a shared `_overlay_report` core (behavior unchanged). Opus-reviewed: marginal math correct + PIT-safe (composite inherits each builder's shift(1)); only deltas are decision-grade. Foundation for G1/G2. 11 tests. | `scripts/walkforward/sleeve_lab.py`, `scripts/walkforward/sleeves.py`, `tests/test_crash_governor.py` |
| 2026-06-14 | (F1b hardening) | **Crash-governor follow-ups (the F1b GO-LIVE MEDIUM/LOW items) resolved.** (1) The applied governor multiplier is now recorded on the per-order `decision_audit` rows (`_audit` gained `size_mult` → `write_decision(size_multiplier=…)`; the two "enter" sites pass `gov_mult`) — a queryable incident-review trail of WHY a day's trend orders were sized down. (2) The in-rebalance macro refresh is now timeout-bounded (`_refresh_macro_history_bounded`: daemon thread + `join(20s)`) so a yfinance network HANG can't block the weekly rebalance — falls through to the cached parquet on timeout (atomic-rename writes → no torn read). Opus-reviewed SAFE; fail-safe contract intact. 4 tests. | `app/live_trading/trend_sleeve.py`, `tests/test_crash_governor.py`, `tests/test_trend_sleeve.py` |
| 2026-06-14 | (CI) | **CI hardening — flaky training-test timeout + NFP local-only failure fixed.** `app/ml/training._windows_to_matrix` now runs SERIAL in-process when `n_workers <= 1` (behaviour-identical to a 1-worker pool, but no subprocess/Manager) — eliminates the nested-ProcessPoolExecutor CPU oversubscription that timed out `test_build_train_matrix_is_non_empty` inside a pytest-xdist worker; production retrains (`--workers 8`) keep the unchanged parallel path. Result-folding extracted to a shared `_consume_result` + `_save_checkpoint` (loop-body cadence preserved for resume parity). `tests/test_phase20.py::TestNfpGate` made hermetic (stub `requests.get` → forces the NFP first-Friday fallback; was hitting live FMP when a local key was present). Opus-reviewed behavior-preserving. | `app/ml/training.py`, `tests/test_phase20.py` |
| 2026-06-14 | (F3) | **Rates duration carry — NEAR-MISS (strongest additive candidate; not promoted).** `app/strategy/carry.py` (`term_spread`, `rates_carry_backtest`: size IEF by the 10y−3m term spread) + `build_rates_carry`/`fetch_yield` (^TNX/^IRX) in `sleeves.py` (declared `risk_premium`). Canonical (IEF/scale 1.5/long-short), 2007→2026 (~3661 OOS): point_SR +0.314, residual-α_t **+2.10** (first genuine positive alpha; beta −0.09), HAC p 0.0998 → Track-A FAIL; Track-B 7/8 (IR +0.386, corr +0.010, ΔSR +0.084) fails only P(ΔSR>0) 0.886<0.90. Misses both bars marginally → FAIL. Robustness grid (12 cfg): edge real+robust (+resid-α all 12; Track-B passes 8/12). Opus review: HONEST — no look-ahead, NO ^TNX/^IRX unit bug (both %, real term spread, 51% interior positions), cost-insensitive. NOT promoted; owner-recommended fresh pre-registration (R7). 7 tests. | `app/strategy/carry.py`, `scripts/walkforward/sleeves.py`, `tests/test_carry.py` |
| 2026-06-14 | (F2) | **ETF relative-value sleeve — FAIL (genuinely orthogonal, no standalone edge).** `app/strategy/etf_relative_value.py` (dollar-neutral log-spread mean-reversion, stateful entry/exit band, 5 pre-registered economically-linked pairs) + `build_etf_relative_value` in `sleeves.py` (declared `diversifier`). Deep history 2007→2026 (~3661 OOS), canonical L=120/entry 1.5/exit 0.5/2bp-leg: point_SR +0.026, HAC p 0.458 → PAPER FAIL; Track-B corr **−0.230** (truly orthogonal) but standalone SR≈0 → IR +0.10, ΔSR −0.011 → FAIL. Grid: every {L,entry} cell below the 0.30 floor net of cost; no per-pair hidden winner. Opus review: HONEST (spread sign correct, PIT clean, cost fair). No promotion. Learning: standalone return — not orthogonality — is the binding constraint. 9 tests. | `app/strategy/etf_relative_value.py`, `scripts/walkforward/sleeves.py`, `tests/test_etf_relative_value.py` |
| 2026-06-14 | (F1b GO-LIVE) | **VIX-term crash governor WIRED INTO THE LIVE TREND SLEEVE (owner-approved).** `trend_sleeve.run_trend_rebalance` scales the sleeve budget `alloc *= _crash_governor_multiplier(db)` (∈ [derisk_to,1.0]); new shared `crash_governor.live_governor_multiplier` (single source of truth with the backtest rule); 4 `pm.crash_governor_*` agent_config flags (enabled default ON). FAIL-SAFE (any data problem → 1.0 = today's behavior; can only reduce exposure; per-name + 80% gross caps still bind). Opus pre-merge review found + fixed 1 HIGH (PIT look-ahead: filter to SETTLED closes before today, matching backtest shift(1)); decision_audit + yfinance-timeout noted as MEDIUM/LOW follow-ups. 50 tests. ⚠️ needs orchestrator restart to load code; reversible via flag. | `app/live_trading/trend_sleeve.py`, `app/strategy/crash_governor.py`, `app/database/agent_config.py`, `tests/test_crash_governor.py`, `tests/test_trend_sleeve.py` |
| 2026-06-14 | (F1b) | **VIX-term crash governor — MODESTLY HELPS (first positive; candidate overlay, owner-gated).** Added the OVERLAY eval path to the Sleeve Lab (`Overlay`/`evaluate_overlay`/`OverlayReport` — book-WITH vs WITHOUT on tail metrics; closes the F0 overlay gap) + `app/strategy/crash_governor.py` (`vix_term_multiplier`: de-risk on VIX>VIX3M backwardation, PIT signal at close t → applied t+1) + `build_vix_term_governor`/`run_governor` in `sleeves.py`. On the live 10-ETF TSMOM book 2007→2026 (n=4891), canonical (derisk_to=0.5, thr=1.0, confirm=1): maxDD −13.9%→−12.1%, Calmar 0.469→0.501, Sharpe 0.723→0.720 (−0.003), COVID DD −10.7%→−6.5%; ~0.5%/yr give-up; de-risks 11% of days. Independent Opus review: HONEST + PIT-verified (un-shift → Sharpe 1.038), no GFC data gap, robust (36-cell grid). **Promotion = owner decision (re-times live exposure); NOT promoted.** 7 tests. | `scripts/walkforward/sleeve_lab.py`, `app/strategy/crash_governor.py`, `scripts/walkforward/sleeves.py`, `tests/test_crash_governor.py` |
| 2026-06-14 | (F1a) | **Calendar/overnight premia sleeves — BOTH FAIL (report-only).** First sleeves through the F0 Lab: `app/strategy/calendar_premia.py` (vectorized PIT-safe `turn_of_month_backtest` + `overnight_premium_backtest`) + `scripts/walkforward/sleeves.py` (the `@register_sleeve` registry + CLI runner + `live_trend_book_returns` Track-B base). Deep history 2007→2026 (~3661 OOS obs), declared `risk_premium`. Verdict: both clear the plausibility SR floor (point_SR 0.320 / 0.344) but miss HAC significance (p 0.097 / 0.087 > 0.05) and decisively fail Track-B vs the trend book (don't diversify; overnight is timed SPY beta). Independent Opus review: HONEST, no bug (PIT clean, costs fair, Track-B fail cost-independent). No promotion; verdict recorded. 15 tests (offline). | `app/strategy/calendar_premia.py`, `scripts/walkforward/sleeves.py`, `tests/test_calendar_premia.py`, `tests/test_sleeves_registry.py` |
| 2026-06-14 | (F0) | **Sleeve Lab — Alpha-v7 F0 foundation.** New `scripts/walkforward/sleeve_lab.py`: a uniform, hardened pipeline taking any candidate return-stream from declaration → Ruler-v2 verdict, retiring the bespoke per-idea `run_*_cpcv` scripts for NEW sleeves. `Sleeve` (validated declaration: label / component_type / PIT net returns / optional spy_prices + n_trials_registered) → `evaluate_sleeve()` runs the IDENTICAL path every time: `SeriesReturnStrategy` → `run_cpcv` (FULL_* constants) → `ruler_v2` Track-A PAPER+CAPITAL → optional Track-B `appraise_track_b` book-delta → uniform `SleeveReport` (+ ASCII `format_sleeve_report`, `to_dict`). `@register_sleeve`/`build_sleeve`/`list_sleeves` registry; `assemble_book()` (F5) combines accepted sleeves via `sleeve_allocator.build_book`. Composes existing proven pieces — **no gate-semantics change; report-only** (promotion stays owner-gated). Opus adversarial deep-dive applied: `overlay` component_type fail-loud-excluded (additive model wrong for it; dedicated path in F1), loud warnings for thin OOS / n_folds<CAPITAL floor / missing spy_prices, assemble_book inner-join coverage warning; KNOWN LIMITATIONS documented (residual-α is SPY-CAPM-only on this path; multi-factor wiring deferred). 20 tests (offline). | `scripts/walkforward/sleeve_lab.py`, `tests/test_sleeve_lab.py` |
| 2026-06-13 | #480 | **Track-B GO-LIVE: wired the run_book_gate dispatcher + flipped `TRACKB_MODE` → `ruler_v2`** (completes the #479 audit's MAJOR-1: the earlier TRACKB flip would have been inert). `run_book_gate._evaluate` now dispatches on `TRACKB_MODE` — `"ruler_v2"`→`track_b_appraisal.appraise_track_b` (trend declared `risk_premium`→regime waived; budget-invariant appraisal IR + block-bootstrap P(ΔSR>0)); `"book_delta"`→legacy gate (retained). New ASCII `format_report(TrackBAppraisalResult)`; ruler_v2 results recorded under their own hypothesis_id (`…-RULERV2-20260613`, parent=the book-delta row). Manual, report-only tool (`decision='park'`, owner-gated, no cron). Live run: tsmom-vs-PEAD → PASS (IR +0.881, P(ΔSR>0) 0.976, regime waived). Opus deep-dive: SHIP. 16 tests. See DECISIONS 2026-06-13 (Track-B GO-LIVE). | `scripts/run_book_gate.py`, `scripts/walkforward/track_b_appraisal.py`, `app/ml/retrain_config.py`, `tests/test_run_book_gate.py`, `tests/test_track_b_appraisal.py` |
| 2026-06-13 | #479 | **GO-LIVE: `GATE_MODE` flipped `significance` → `ruler_v2` (Ruler v2 is now the production Track-A gate).** Alpha-v7 Phase B. After R4 came back CLEAN on the full control set (significant positives tsmom_4y/19y pass Ruler-v2 PAPER via the diversifier regime waiver + significance; all 5 true-nulls fail via the new PAPER HAC-significance floor; leaky rejected; xmom_12_1 reclassified `positive_alpha`→`known_marginal` as a logged correct-reject), the owner approved the flip. PAPER = plausibility + light one-sided HAC-SR significance floor (`RULERV2_PAPER_MAX_HAC_P=0.05`, pooled-OOS) + worst-regime waiver for declared diversifiers/risk-premia; CAPITAL = Bayesian posterior≥0.95 (trial-shrunk prior, replaces saturated DSR) + STRUCTURAL live-paper + multi-factor residual-α t≥2.0 + bootstrap P(SR>0)≥0.95 + (PBO≤0.50 if M>1) + power floor (n_obs≥504, n_folds≥10). Legacy `significance`/`mean_sharpe` branches RETAINED (reachable via `significance_gate_*`/explicit mode). `gate_calibration` DECOUPLED from GATE_MODE (significance OC columns via `significance_gate_*` directly) so the calibration/recalibration rule stays correct under the new default. `TRACKB_MODE` stayed `book_delta` at this PR (flipping it would have been inert — no dispatcher; **superseded same day by #480** which wired the runner + flipped it to `ruler_v2`). Independent Opus go-live audit: mechanically correct, no live break (all `gate_detail` consumers iterate generically), cleanly reversible, R4 strictly CLEAN (no carve-out; an earlier carve-out was caught+reverted as goalpost-moving). Full suite under the flip: only the known local-only NFP test fails (3398 passed). See DECISIONS 2026-06-13 (GO-LIVE) + RULER_V2_DESIGN.md + §7.0. | `app/ml/retrain_config.py`, `scripts/walkforward/gate_calibration.py`, `tests/test_gate_calibration.py`, `tests/test_track_b_appraisal.py` |
| 2026-06-12 | #456 | **Alpha-v6 P3 — event panel + event-LEVEL inference instrument; H1 verdict = PEAD DEMOTED.** New `scripts/walkforward/event_inference.py`: Cameron-Gelbach-Miller **two-way (announce_date × firm) cluster-robust OLS** (`twoway_cluster_ols`, intercept-only mean test for H1; `decile_report`, `loco_robustness`), **validated to the published Petersen (2009) SEs** (two-way 0.0535580 vs 0.053561) — this is now the **Track-A significance instrument for event/rules-based strategies** (per blueprint §3b: event-panel clustered inference is the *significance* check; full-coverage CPCV becomes the *robustness/coverage* check; both required for promotion). New `app/research/event_panel.py` + `scripts/build_event_panel.py` → `data/event_panel.parquet` (one row/event, R1K 2019→2026, PIT, knowable-date discipline, `QF_SUSPECT_BARS` excludes unhealable split-cliff names). New `scripts/run_event_panel_inference.py` (registry-enforced, one-shot R4, confirmatory coverage gate + panel SHA pin). Also: `registry_enforcement.begin_run` now rejects `--exploratory` + `--hypothesis-id` together. **H1 RESULT: PRIMARY 10d SPY-hedged mean −8.3bp, two-way t=−0.77, p=0.7804 → DEMOTE** (PEAD not an event-level edge; live book = trend-plus-cash). No production gate threshold changed. 51 + regression tests. See DECISIONS 2026-06-12 + ML_EXPERIMENT_LOG. | `scripts/walkforward/event_inference.py`, `app/research/event_panel.py`, `scripts/build_event_panel.py`, `scripts/run_event_panel_inference.py`, `scripts/walkforward/registry_enforcement.py`, `tests/test_event_inference.py`, `tests/test_event_panel.py`, `tests/test_run_event_panel_inference.py`, `tests/fixtures/petersen_test_data.txt` |
