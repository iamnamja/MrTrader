...
scripts/
  ...
  walkforward/
    ...
    sleeves.py                  — the SLEEVE REGISTRY (F1+): @register_sleeve builders (turn_of_month, overnight, …) + the VIX-term governor overlay (build_vix_term_governor/run_governor) + CLI (python -m scripts.walkforward.sleeves [name|governor]). Backtests live in app/strategy/ (calendar_premia.py, crash_governor.py).
    strategies/
