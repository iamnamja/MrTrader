...
scripts/
  ...
  walkforward/
    ...
    sleeve_lab.py               — Sleeve Lab (Alpha-v7 F0): uniform Sleeve→run_cpcv→ruler_v2 Track-A(PAPER+CAPITAL)→[Track-B]→SleeveReport + sleeve registry; assemble_book() for F5. Report-only; replaces bespoke run_*_cpcv scripts for NEW sleeves.
    sleeves.py                  — the SLEEVE REGISTRY (F1+): concrete @register_sleeve builders (turn_of_month, overnight, …) + CLI runner (python -m scripts.walkforward.sleeves). Backtests live in app/strategy/ (e.g. calendar_premia.py).
    strategies/
