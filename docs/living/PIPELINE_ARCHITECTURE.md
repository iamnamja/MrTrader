...
walkforward_tier3.run_intraday_walkforward()
  ...
  ├─ Loads per-symbol DAILY bars (52w-position / vol-percentile features) from
  │    INTRADAY_DAILY_FEATURE_PROVIDER (default "yfinance", full multi-year history) —
  │    NOT from the 5-min provider. Pre-#DAILYPROV these came from Alpaca, which caps
  │    at ~100 recent daily bars → 52w/vol features silently 0.5-defaulted (see #DAILYPROV).
