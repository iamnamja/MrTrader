...
| **P3** | **Earnings-event panel + PEAD v2** (centerpiece): one-row-per-event table; **event-LEVEL inference** (two-way clustered announce-day×firm); pre-registered H1 (re-adjudicate live PEAD), H2 (continuous reaction-ratio — settles OPT-5), H3 (continuous options-conditioned scorecard, NOT XGBoost). | A | L ~3–4wk | 🔨 **H1 DONE → ❌ DEMOTE (#456)**: panel (21,330 events / 9,774 qual, 2019→2026) + CGM two-way inference (Petersen-validated) built; H1 10d SPY-hedged p=0.7804 → PEAD not an event-level edge → book=trend-plus-cash. Event panel + inference = the standing event-hypothesis instrument. **H2/H3 NOT auto-run** (were PEAD-improvement hypotheses; owner call whether to run as pure research now PEAD is demoted). |
| **P4** | **Options-as-equity-signal XS sleeve**: CPIV / 25Δ put-skew / O-S volume / term-slope / IV-RV decile L/S, executed in equities via the existing dollar-neutral engine. Kill if simple sorts fail net of costs (NOT an XS-ML revival). | A | M–L ~2–3wk | 🔨 **P4a DONE (2026-06-12): data layer + pre-registration.** Feature table + quality filter shipped (see P2); **H4a–H4e PRE-REGISTERED** (`scripts/preregister_options_xs_features.py`, `preregistered_at=2026-06-12T12:00Z`; CPIV→+, skew→−, O-S→−, term-slope→+, IV/RV→−; kill = simple decile sorts show nothing net of costs → CLOSE). **NEXT = the 5 confirmatory R4 L/S runs** (weekly dollar-neutral, week-clustered t≥2 + monotone deciles + cost-net residual α; `run_at>2026-06-12T12:00Z`, zero live capital). |
