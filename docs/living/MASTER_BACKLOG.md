...
**SSOT:** [`docs/reference/COMPOUND_AND_HARDEN_PROGRAM_2026-07-07.md`](../reference/COMPOUND_AND_HARDEN_PROGRAM_2026-07-07.md) (verdict: `docs/reference/prompts/20260707_Comp_Review/SYNTHESIS_AND_DECISION.md`; DECISIONS 2026-07-07). After ~28 evaluated families → one live edge (ETF trend) + cash, and the futures book failed the diversification gate, the 5-LLM panel (DeepSeek/ChatGPT/Gemini/Claude/Grok) was **unanimous: STOP the broad second-edge hunt; make the ONE edge antifragile + condition-reactive and let it compound.** Phases: **CH0** baseline + live-forward scorecard ✅ DONE (2026-07-07; baseline CPCV mean_sharpe 0.7009 frozen + governor scorecard live from the 07-13 enforce rebalance) → **CH1** per-name correlation/heat/concentration gate on the live path ✅ SHADOW-LANDED (2026-07-07; `per_name_gate.py`, default shadow; correlation enforce-threshold calibrated from the soak before any flip) — **soak tracker landed 2026-07-21:** each LIVE rebalance now persists the gate's realized metrics (`per_name_metrics` on the `trend_backval_daily` row) and `back_validation.per_name_soak_report()` summarizes the observed book-corr distribution + would-block counts at 0.80/0.85/0.90/0.95, so the enforce threshold is calibrated from data, not a guessed constant. 07-20 first clean current-code obs: book_corr 0.35, max_name_w 0.08 — miles from the 0.90 gate. **✅ FLIPPED TO ENFORCE 2026-09-07** after 6 clean Mondays (07-27 → 08-31; 0 would-blocks at every threshold 0.80–0.95). First live enforce rebalance **Tue 2026-09-08 CLEAN** — book_corr 0.244, max_name_w 0.064, blocked=0. (The flip also exposed a stale expectation in the enforce-health verifier, fixed 2026-09-09 — DECISIONS.) → **CH2** the 3 NEW antifragile sizing pieces (each gated by the DUAL gate — beat constant-gross OOS mean_sharpe, params→DSR, AND no BEAR-regime-Sharpe regression) ✅ COMPLETE (2026-07-08; gate harness `app/research/ch2_sizing.py` built; **all 3 KILLED — antifragile sizing does NOT beat constant-gross:** CH2b correlation −0.040 (direction-blind), CH2c whipsaw governor +0.0006 n.s. (beats the plain governor, flagged it for CH3), CH2a trend-strength/breadth −0.07/−0.19 (strength is contrarian to forward Sharpe). Positive result — keep the trend book's sizing SIMPLE; no live change) → **CH3** regime-conditional decomposition ✅ DONE (2026-07-08; `scripts/ch3_regime_diagnostic.py`: **NEITHER parked strategy is a conditional diversifier — both CLOSED** (sector_rotation decorrelates in BEAR 0.40→0.14 but LOSES −34%/yr there; credit_timing collinear 0.33); no CH4 candidate from them. **The live plain crash governor HELPS on its drawdown mandate** (maxDD −1.8pp, COVID −4.25%) → KEEP it, resolving the CH2c flag; no live change) → **CH4** the ONE terminating pre-registered search = the ranging-market MR sleeve ✅ DONE (2026-07-08; `app/research/ch4_mr_sleeve.py`, pre-registered `CH4_MR_PREREGISTRATION_2026-07-08.md`): **KILLED** — primary fails 3/5 gates (Track-A 0.012, off-regime −0.11, Track-B t −0.52); ranging regime rare (2% active); the one good-looking config was a detection-lag mirage (CPCV 0.82→0.16 under a 2d regime lag), and primary-only decision meant it couldn't flip the verdict. **★ 12-month hunting MORATORIUM now IN EFFECT (to 2027-07-08).** → **CH5 ← NOW** live accrual + pre-committed 12-mo review (2027-07-08). **★ COMPOUND-AND-HARDEN COMPLETE through CH4** — edge = trend+cash; operate it simply. Futures-live = SHELVED. This SUPERSEDES the futures-hunt direction; the Alpha-v10 "harden, don't hunt" program below is its parent lineage.
...
| **H2** | wire the **kill-switch state machine**; flip `whole_book_gate_mode` shadow→enforce after H10 + a clean shadow week | 🔭 **code-half SHADOW DONE 2026-06-25** — `kill_switch_sm` singleton + `evaluate_new_risk` consult wired at trend/cash/RM gates; auto-triggers fed (recon-FAIL_CLOSED when recon enforce; heartbeat); flag `pm.kill_switch_sm_mode` default shadow; Opus SHIP-ABLE (zero live change in shadow). `pm.whole_book_gate_mode`→enforce is DONE (2026-07-06, LIVE). **✅ reduce-only refinement DONE (2026-07-21):** `evaluate_rebalance(mode)` classifier (NORMAL→full / HALT_NEW_RISK→reduce_only / CANCEL_ONLY+→blocked) replaces the coarse whole-rebalance block at the trend+cash sleeves — under HALT_NEW_RISK protective SELLS still shed risk (trend: sells kept, buys suppressed; cash: buffer-replenish sell kept, T-bill deploy suppressed) so gross can only fall; CANCEL_ONLY+ blocks. Shadow still runs FULL (zero live change until the flip). RM keeps `evaluate_new_risk` (per-order primitive). **✅ `pm.kill_switch_sm_mode`→enforce FLIPPED LIVE 2026-07-21** (owner-approved). SM dormant/`NORMAL`, so no behavior change until an auto-trigger (dead-man / recon-FAIL) escalates it → then the rebalance runs reduce-only. Daemon reads the mode live (no restart). Caveat: not yet exercised by a real drawdown. Revert: `set_agent_config(db,'pm.kill_switch_sm_mode','shadow')`. **H2 COMPLETE.** |
