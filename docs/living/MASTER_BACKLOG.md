...
| **P2** | **Options feature layer**: persist computed IV/greeks (one pass over 112.8M bars), surface-quality reader, BMO/AMC event-time-aware snapshots, daily per-underlying feature table, R1K-union survivorship audit. | infra | M ~2wk | 🔨 **greeks backfill BUILT + STARTED (#455)** — `backfill_computed_greeks.py` (European-warmstart American refine, ~0.6h; as-traded Polygon `adjusted=false` closes) → `data/options_greeks/`. Surface-quality reader / feature table / BMO-AMC snapshots / survivorship audit still TODO. |
