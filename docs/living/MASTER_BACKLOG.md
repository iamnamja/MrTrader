...
| **F1** | **Structural premia + crash governor** — turn-of-month / FOMC / overnight-gap sleeves + a VIX-term de-risking overlay (governor). | Most orthogonal-to-trend, most powered (high event count), cheapest, owned data, no spread wall. | ~1–2wk | 🟡 **IN PROGRESS** — **F1a DONE 2026-06-14: turn_of_month + overnight BOTH FAIL** (clear plausibility SR floor but miss HAC significance p>0.05, and decisively fail Track-B — don't diversify the trend book; overnight is timed SPY beta. Opus-verified honest. `app/strategy/calendar_premia.py` + `scripts/walkforward/sleeves.py`; no promotion). NEXT F1b: overlay eval path + VIX-term crash governor. F1c (cond.): FOMC drift if a clean date list is feasible. |
