...
```
**FEATURES (Alpha-v6 P2, 2026-06-11):** persisted computed greeks (`data/options_greeks/`, `scripts/backfill_computed_greeks.py`) + nightly NBBO snapshots (`data/options_spread_obs.parquet`, Alpaca) — turns the 112.8M-bar store into a queryable IV/greeks/spread feature asset so panel/XS queries are joins, not pricing runs.

**FEATURE TABLE + P4 PRE-REGISTRATION (Alpha-v6 P4a, 2026-06-12):** the daily options FEATURE TABLE (`app/data/options_features.py` + `scripts/build_options_features.py` → `data/options_features.parquet`, one row per (underlying, date)) lifts the greeks store into the five cross-sectional EQUITY-signal features — `cpiv_matched_delta`, `skew_25d_put`, `term_slope_30_60`, `iv_rv_20d_ratio`, `opt_share_volume_ratio` (+ atm_iv_30d, implied_move_front, put_call_volume_ratio, opt_volume_z) — with a frozen valid-contract quality contract and PIT throughout (holiday-aware knowable_date, split-adjusted RV). `app/data/options_quality.py` is the PIT coverage-floor universe filter. **H4a–H4e** are PRE-REGISTERED (`scripts/preregister_options_xs_features.py`, `preregistered_at=2026-06-12T12:00Z`): the **options-as-signal** thesis — does each feature carry a cross-sectional equity edge, judged as a weekly dollar-neutral L/S sleeve (information at equity cost, NOT an options trade), week-clustered t≥2 + monotone deciles + positive cost-net residual alpha. **Kill rule: simple decile sorts show nothing net of costs → CLOSE the line (NOT a revival of the dead XS-ML).** See OPTIONS_DATA §(feature table) + DECISIONS/ML_EXPERIMENT_LOG 2026-06-12.

**▶️ H4a–H4e VERDICT (2026-06-12) — ❌ ALL 5 KILL: options-as-signal cross-sectional equity edge is DEAD.** The full-4y/208-week confirmatory runs (`scripts/run_options_xs_cpcv.py`, recorded R4) — week-clustered t / net spread / residual-α t: **H4a** CPIV −2.70/−57bp/−2.79, **H4b** skew −4.10/−55bp/−4.30, **H4c** put-heavy O/S −4.43/−50bp/−3.16, **H4d** term-slope −2.83/−48bp/−2.78, **H4e** IV/RV −0.12/−2bp/−0.72. H4a–H4d significantly NEGATIVE (the hypothesized books lose — academic signs don't hold 2022–26 at equity cost); H4e noise. The inverse is NOT tradeable (post-hoc sign-mining; Opus deep-dive = 2022–23 growth-crash regime). **All five options-as-signal lines CLOSED.** Options confirmed (again) as a DATA asset, not a tradeable equity signal — the live book stays trend-only (25%) + cash. Options data now serves event-conditioning research only (H2/H3). See DECISIONS 2026-06-12.
...
| **P2-4** | **Calibrated, moneyness/DTE-aware spread cost model** (`app/options/spread_model.py::CalibratedSpreadModel` + `CalibratedOptionsSpreadCostModel`): empirical median relative spread bucketed by (underlying×moneyness×DTE×call/put) from the live NBBO log, hierarchical fallback → conservative p75 global; sim passes contract context to the cost call; `scripts/calibrate_option_spreads.py`. Flat 1% stays default (calibrated opt-in). 16 tests. | ✅ framework shipped 2026-06-16 — **calibration PRELIMINARY** (~4 days indicative-feed NBBO; moneyness/DTE-aware not IV-bucketed; ANACHRONISTIC for multi-year history via `covers_date()` guard; **NOT wired to any live VRP verdict** — re-run as data accrues). SPY ATM 30D ≈ 0.86% half-of-premium, deep-OTM ≈ 33%. |
