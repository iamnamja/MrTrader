...
**NEXT → Phase 3: regime-aware allocator over {PEAD, trend}** — the live multi-sleeve book (vol-weight by marginal book risk + continuous regime tilt; trend's slow-bear-positive / fast-shock-negative profile is a key allocator input). Live wiring of the trend sleeve happens here (its natural home), validated book-level (must beat static weights net of turnover). (Phase 0 remainder — sequential-WF baseline, fold-coverage report, gate recalibration, freeze dead XS-ML — runs alongside as harness hygiene.)
