...
**Last updated:** 2026-06-12 (**H1 RUN → PEAD DEMOTED at event level (p=0.78). ✅ PEAD FLIPPED OFF LIVE + uvicorn restarted (overnight) → live book = trend-only (40%) + cash. P0+P1c+P2+P3-H1 shipped (#454/#455/#456) + P4a options feature table + H4a–H4e pre-registered. trend weight reconciled 40%→25%. P4 H4a–H4e RUN → ALL 5 KILL (options-as-signal equity edge dead). NEXT: H2/H3 event-conditioned research (event panel now options-enriched)**)
