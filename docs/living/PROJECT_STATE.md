...
**🟢 TRACK B SHIPPED (2026-06-04):** B1 (realized-Sharpe EOD pipeline) + B2 (Friday rollup) were **already built/scheduled** (16:30 ET, 12 tests) — the self-certification clock is live. **B4 aggressive paper ramp** built + merged: config-driven `pm.pead_size_mult`=3.0 + `pm.pead_max_position_pct`=0.10 (live-tunable via agent_config), PEAD-specific `apply_pead_size_ramp`; RM made PEAD-aware so the 10% per-name cap isn't clipped to the global 5% (aggregate still bounded by the 80% gross cap); ADV-participation instrumentation added (slippage already per-fill). 19 tests; PAPER ONLY. **Deploys at the next uvicorn restart.** Tune the ramp live by editing `pm.pead_size_mult` / `pm.pead_max_position_pct` in agent_config (no redeploy).
