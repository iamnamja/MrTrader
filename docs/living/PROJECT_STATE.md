...
## 🧭 NOW (2026-06-14, executing): Alpha-v7 F0 Sleeve Lab CORE LANDED — F1+ sleeves build through it

**Owner gave go-ahead to execute the full F0→F5 plan autonomously (Opus each step; document/email/merge; promote-to-live stays owner-gated).** **F0 CORE LANDED:** `scripts/walkforward/sleeve_lab.py` — the uniform, hardened pipeline (`Sleeve` validated declaration → `evaluate_sleeve`: `SeriesReturnStrategy`→`run_cpcv`→Ruler-v2 Track-A PAPER+CAPITAL→optional Track-B→`SleeveReport`; `@register_sleeve` registry; `assemble_book` for F5; ASCII report). Composes proven pieces, **report-only, no gate-semantics change**. Opus adversarial deep-dive applied (overlay fail-loud-excluded pending a dedicated path in F1; loud thin-OOS / n_folds<10 / no-spy warnings; assemble_book coverage warning; KNOWN LIMITATIONS doc'd — residual-α SPY-CAPM-only for now). 20 offline tests. **NEXT: F1** structural/calendar/overnight premia sleeves + the VIX-term crash governor (the governor needs the overlay eval path). Live book unchanged (trend-only 25% + cash).
