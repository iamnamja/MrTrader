...
- `data/options_spread_obs.parquet` (**nightly NBBO snapshots — Alpha-v6 P1c**, `scripts/log_options_nbbo.py`): per (contract, obs_date) `feed, bid, ask, mid, spread_pct, bid_size, ask_size, iv, oi, day_close, day_volume, underlying_price, moneyness, dte`. Logged 15:55 ET (trading days only) from the **Alpaca** options snapshot (`feed=indicative`, free) — **the Polygon plan serves NO NBBO** (see §1). Feeds `app/options/spread_model.py::CalibratedSpreadModel` (**built P2-4, 2026-06-16**): `scripts/calibrate_option_spreads.py` → `data/options_spread_model.json` (median relative spread by underlying×moneyness×DTE×call/put, conservative p75 fallback). **Calibration is PRELIMINARY until ~4–6 weeks accumulate** — `covers_date()` flags the (currently ~days-wide) window; NOT wired to any live VRP verdict. The orchestrator **auto-recalibrates nightly** after the NBBO snapshot (keeps `options_spread_model.json` fresh) and emails `options_spread_mature` **once** the window crosses `MATURE_MIN_DAYS` (20 trading days) → re-assess the VRP go/no-go then.
...
- **Event-time options join** (P3, for H2/H3) — `app/research/event_options_join.py` + `scripts/enrich_event_panel_options.py` lift this feature table onto each earnings event (`data/event_panel.parquet` OPTION_COLUMNS): a PIT as-of join taking the **pre-event snapshot** = the last chain knowable strictly before the announce day (gated on the holiday-aware `knowable_date`; UNK BMO/AMC → conservative). Derives `reaction_ratio` (|announce-day move| / pre-event implied move), `iv_runup_t10_t1`, `post_iv_retention_t1`, and the `*_pre` surface columns. ~45% event coverage (options data starts 2022). Opus-deep-dived (no look-ahead: the forward return starts at announce+1 open). This is the instrument the event-conditioned hypotheses (H2/H3) adjudicate on.
