...
- ⏳ **Re-audit drives the rest:** the remaining research/data findings (data-provider filing-date PIT, ruler/null_zoo/inference statistics, ml-regime BenignGate/NIS/vix_term, ml-training WF-CV ordering, data-market cache split-adjust, options-short minors) + all Wave-5 MINORs are being surfaced/confirmed by the **whole-app re-audit** (next), then fixed-and-iterated until zero confirmed — per the owner's "re-run the check, keep iterating until no bugs" directive. (Many MINORs are in parked/dead code; the re-audit's adversarial verify decides which genuinely remain.)
