# External Quant Review — MrTrader Options & Alpha Pipeline
...
- **2× spread stress** — essential for options costs.
...
`N_eff_trials = n_folds * (n_folds-1) / 2` for the number of distinct train/test splits. Then compute DSR via the standard formula (Marcos López de Prado). This gives a honest, less conservative multiplicity penalty.
...
- If portfolio delta > 0.5 × equity, reduce exposure.
